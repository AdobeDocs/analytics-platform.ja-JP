...
| 計算指標 | セグメントを含む複雑な数式に基づく可能性のある、顧客が作成したカスタム指標の評価。 | 例：赤い靴を購入したユーザーの数。 |
