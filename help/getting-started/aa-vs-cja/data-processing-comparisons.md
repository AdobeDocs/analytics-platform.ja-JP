...
また、Analytics ソースコネクタで作成されたデータセットでは、指標としての訪問もセッションも使用できないので、比較をおこなうには、クエリロジックでセッションを定義する必要があります。
...
| 訪問者レベルのロジック | 人物レベルで適用されるロジック。 | 例：クロスデバイス/クロスチャネルでのユーザーステッチ。 |
| セグメント（フィルター）ロジック | イベント/訪問/人（イベント/セッション/人）セグメント（フィルター）ルールの評価。 | 例：赤い靴を購入した人物。 |
...
| 派生フィールド | 論理は、データビューでのコンポーネントの定義の一部として、スキーマまたは標準フィールドに適用されます。 | 例：新しいマーケティングチャネルディメンションの作成 |

{style="table-layout:auto"}

時間の経過と共に、Adobe Analyticsと現在のCustomer Journey Analyticsは、レポートの実行時に訪問レベルとユーザーレベルのデータロジックを実行できるようになり、柔軟性が向上しました。
