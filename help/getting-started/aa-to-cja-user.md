...
* [Report Builder](/help/report-builder/rb-overview.md) には、MS Windows、macOS および Excel（web バージョン）で動作する新しいインターフェイスが用意されています。 （このバージョンのReport Builder以前は、VMwareで実行しない限り、Macでは使用できません）。 このバージョンでは、従来のAA データリクエストはまだサポートされていません。
