...
スキーマを構築する際は、事実に固執します。 例えば、`error.type = "validation"`、`user.isLoggedIn = true`、`checkout.step = "shipping"` です。 データビューの指標をカウントと定義し、それらのファクトに対するフィルター適用済みカウントを定義します。 次に例を示します。
