...
>title="タグへの XDM データ収集ロジックの追加"
>abstract="サイトにローダータグをインストールすると、ルールとデータ要素を追加して、アドビに送信する XDM オブジェクトにデータを入力できます。 アドビでは、タグの設定方法を追跡するために、ソリューションデザインドキュメントを保守することをお勧めします。<br><br>この手順では、プロパティのすべての Analytics ロジックを設定する必要があるので、多くの作業が必要になります。正しいタグルールを確立し、テストして、サイトにデプロイするには、1 か月以上かかることを想定してください。"
