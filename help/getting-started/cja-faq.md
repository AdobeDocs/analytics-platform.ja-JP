...
| データセットからのバッチの削除( [!UICONTROL Adobe Experience Platform])? | バッチが [!UICONTROL Adobe Experience Platform] データセットに追加した場合、同じバッチが、その特定のバッチを含むCJA接続から削除されます。  CJAには、 [!UICONTROL Adobe Experience Platform]. |
