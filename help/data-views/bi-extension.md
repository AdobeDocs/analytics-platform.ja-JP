...
1. 期限切れでない資格情報がCustomer Journey Analyticsの製品および製品プロファイルで使用できることを確認します。 組織で次の手順を実行するには、システム管理者である必要があります。
   ...
   1. 有効期限のない資格情報がAPI資格情報のリストに追加されていることを確認します。
      1. 上部メニューから「**[!UICONTROL ユーザー]**」を選択します。
      1. 左側のパネルから「**[!UICONTROL API資格情報]**」を選択します。
      1. 新しい資格情報または移行された期限切れでない資格情報を一覧表示し、**[!UICONTROL EQS-...]**&#x200B;で始める必要があります。

      1. 期限切れでないAPI資格情報がCustomer Journey Analytics製品およびプロファイルにアクセスできることを確認します。

         1. 有効期限のないAPI資格情報&#x200B;**[!UICONTROL EQS-...]**&#x200B;について、![ProductDetails](/help/assets/icons/ProductDetails.svg)を選択します。
         1. **[!UICONTROL EQS-...]**&#x200B;製品の詳細ペインから、![詳細](/help/assets/icons/More.svg)を選択し、**[!UICONTROL API資格情報の編集]**&#x200B;を選択します。
         1. **[!UICONTROL API資格情報を編集]** ダイアログで、**[!UICONTROL 割り当てられたプロファイル]**&#x200B;を検証します。 Customer Journey Analytics製品が表示されない場合：
            1. ![Add](/help/assets/icons/Add.svg)を選択し、**[!UICONTROL Customer Journey Analytics]**&#x200B;を選択します。
            1. クエリサービスとBI拡張機能に対するアクセス権を提供するユーザーを含む1つ以上の製品プロファイルを選択します。
            1. 「**[!UICONTROL 適用]**」を選択します。

1. Experience Platform クエリサービスに有効期限のないAPI資格情報が表示されていることを確認します。

   1. ![App](/help/assets/icons/Apps.svg)から&#x200B;**[!UICONTROL Experience Platform]**&#x200B;を選択します。
   1. 左側のパネルから「**[!UICONTROL クエリ]**」を選択します。
   1. 上部メニューから「**[!UICONTROL 資格情報]**」を選択します。
   1. 有効期限のないAPI資格情報は、**[!UICONTROL 有効期限のない資格情報]** リストに手順1で指定した名前を使用して表示されます。
...
FROM (
  ...
  FROM dv1
  WHERE `timestamp` BETWEEN '2022-01-01' AND '2022-01-02'</br>  DIM1でグループ化
)
...
`SUM`または`COUNT`関数に`IF`または`CASE`句を埋め込んで、選択した指標に固有の追加のセグメントを追加できます。 これらの句を追加することは、Workspace レポートテーブルの指標列にセグメントを適用することと似ています。
...
`daterange`特別な列は`timestamp`と同様に機能しますが、セグメンテーションは全日に制限されます。 `daterange` もオプションで、`timestamp` と同じ範囲のデフォルトが設定されます。
また、`daterange` フィールドを日時関数で使用して、イベントの日付を解析したり切り捨てたりすることもできます。

`daterangeName`特殊列を使用すると、`Last Quarter`のような名前付き日付範囲を使用してクエリをセグメント化できます。
...
`filterId`特殊列はオプションであり、外部で定義されたセグメントをクエリに適用するために使用されます。 外部で定義されたセグメントをクエリに適用することは、Workspaceのパネルでセグメントをドラッグすることと似ています。 `AND`様がセグメント IDを複数使用できます。

`filterId`と共に、`filterName`を使用して、IDの代わりにセグメントの名前を使用できます。
...
1. セグメントに含める外部で定義された`filterId`または`filterName`を検索します。

1. 残りの式をアドホックセグメントに変換します。

この処理は、`WHERE` 句の最初のレベルの `AND` を解析することによって行われます。 最上位の各 `AND` 式は、上記のいずれかと一致する必要があります。 `AND`の最初のレベルよりも深いレベル、または`WHERE`句が最上位の`OR`を使用する場合は、アドホックセグメントとして処理されます。
...
| [エクストラクト](https://spark.apache.org/docs/latest/api/sql/index.html#extract) | ``SELECT EXTRACT(MONTH FROM `timestamp`)`` | 渡されたフィールドで動的ディメンション ID を生成します。 この関数の一部では、わかりやすい名前ではなく数値が必要なので、値の代わりに項目 ID を使用します。<br/> サポートされている部分：<br>- キーワード：`YEAR`、`MONTH`、`DAYOFMONTH`、`DAYOFWEEK`、`DAYOFYEAR`、`WEEK`、`QUARTER`、`HOUR`、`MINUTE`。<br/> – 文字列：`'YEAR'`、`'Y'`、`'MONTH'`、`'M'`、`'DAYOFMONTH'`、`'DAY'`、`'D'`、`'DAYOFWEEK'`、`'DOW'`、`'DAYOFYEAR'`、`'WEEK'`、`'WOY`、`'W'`、`'DOY'` `'QUARTER'`、`'QOY'`、`'Q'`、`'HOUR'`または`'MINUTE'`。 |
...
| 関数 | 例 | 詳細 |
|---|---|---|
| 最小値() と最大値() | ``MIN(daterange)`` または<br/> ``MAX(daterange)`` | `timestamp`、`daterange`、`daterangeday` などの `daterangeX` のいずれかに `MIN()` を実行すると、2 年前が返されます。<br/><br/> `timestamp`、`daterange`、または`daterangeday`などの`daterangeX`の`MAX()`は、現在の日時を返します。<br/><br/>`MIN()` または、他のディメンション、指標、または式の`MAX()`は0を返します。 |
