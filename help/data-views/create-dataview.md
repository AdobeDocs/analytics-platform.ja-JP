...
| [!UICONTROL コンテナ] | ここでコンテナの名前を変更して、このデータビューに基づくWorkspaceプロジェクトでのコンテナの表示方法を決定できます。 コンテナは、フィルターやフォールアウト/フローなどで使用し、範囲やコンテキストの範囲や狭さを定義します。 [詳細情報](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-components/cja-filters/filters-overview.html?lang=ja#filter-containers) |
...
| [!UICONTROL スキーマデータタイプ] | コンポーネントが文字列、整数などかどうかを示します。 |
