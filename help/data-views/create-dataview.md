...
| [!UICONTROL 小文字] | *新規*  - 「文字列」タイプのディメンション。この設定を使用すると、ディメンション値の大文字と小文字をCustomer Journey Analyticsとして扱うかどうかを制御できます。 値は同じで大文字と小文字が異なる行の重複排除が可能です。 「**[!UICONTROL 小文字]**」をオンにした場合、同じ値を持つディメンションのすべてのインスタンスが小文字としてレポートされます。 **not**&#x200B;チェックを[!UICONTROL 小文字]と&#x200B;**do**&#x200B;チェックをオンにした場合の動作を示します。 左の表で、「liverpool」、「Liverpool」、「LIVERPOOL」がレポートで3つの行項目を生成することに注意してください。 右の表では、同じ値が重複排除され、1つの行項目（<br>![大文字と小文字を区別するディメンション](assets/case-sens-workspace.png)）に該当します。 |
...
| [!UICONTROL 「値なし」を値として扱う] | この設定により、データ内の空白の値が[!UICONTROL で指定したテキストに置き換えられます。表示されている場合は、「値なし」を呼び出します。]. 例えば、モバイルデバイスのタイプをディメンションとして指定した場合、「**[!UICONTROL 値なし]**」項目の名前を「デスクトップ」に変更できます。 このフィールドをカスタム値に変更すると、カスタム値は正しい文字列値として扱われます。 したがって、このフィールドに値「Red」を入力すると、データ自体に表示される文字列「Red」のインスタンスが、指定した行項目の下にロールします。 |
...
| [!UICONTROL 配分] | ディメンションで永続性のために使用する配分モデルを指定できます。オプションは、「[!UICONTROL 最新の]」、「[!UICONTROL 元の]」、「[!UICONTROL インスタンス]」、「[!UICONTROL すべて]」です。値を保持する場合は、ここで設定します。 設定できる永続性の最大値は90日です。 また、「[!UICONTROL 無期限]」というオプションではありません。 |
