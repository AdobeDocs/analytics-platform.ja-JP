...
| [!UICONTROL 小文字] | *新規* - 「文字列」タイプのディメンション。この設定を使用すると、Customer Journey Analytics でディメンション値を大文字と小文字を区別して扱うかどうかを制御できます。これにより、値は同じで大文字と小文字が異なる行の重複排除が可能になります。「 **[!UICONTROL 小文字]** 」のチェックをオンにした場合、同じ値を持つディメンションのすべてのインスタンスが小文字としてレポートされます。このスクリーンショットは、「 [!UICONTROL 小文字] 」のチェックを **オンにしない場合** と **オンにした場合** の動作を示しています。左の表で、「liverpool」、「Liverpool」、「LIVERPOOL」がレポートで 3 つの行項目に分かれることに注目してください。右の表では、同じ値が重複排除され、1 つの行項目に分類されます。（<br>![大文字と小文字を区別するディメンション](assets/case-sens-workspace.png)） |
