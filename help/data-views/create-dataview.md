...
データ表示を作成する前に、 [experience platformデータセットへの1つ以上の接続の設定](/help/connections/create-connection.md).

## 設定する

1. Customer Journey Analyticsで、 **[!UICONTROL データ表示]** タブをクリックします。

1. クリック **[!UICONTROL 追加]** をクリックして、データ表示を追加し、その設定を指定します。

   | セッションの設定 | 定義 |
   |---|---|
   | 接続 | このフィールドは、データ表示を、前に確立した接続にリンクします。この接続には、 [!UICONTROL Experience Platform] データセット/秒 |
...
1. 次に、コンポーネント（ディメンション、指標）をデータビューに追加します（仮想レポートスイートのキュレーションと同様）。データセット内の各フィールドがディメンションまたは指標に変換されるようになりました。ディメンションと指標をパネルまたは **[!UICONTROL すべて選択]** をクリックして、すべてのコンポーネントを追加します。

   ![](assets/add-all-components.png)

1. 「 **[!UICONTROL 追加コンポーネント]** タブをクリックして、データ表示にディメンションと指標を追加します。
