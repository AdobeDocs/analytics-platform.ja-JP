...
- リファラーが検索エンジンからのリファラーで、ページに `ps_` を含むクエリ文字列値 `cid` ある場合、マーケティングチャネルは [!DNL *有料検索*] として識別される必要があります。
