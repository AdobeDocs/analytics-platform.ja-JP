...
| [!UICONTROL セッション開始] | 指標 | セッションの最初のイベントとなったイベントの数。フィルター定義で使用する場合（例：「[!UICONTROL セッション開始]が存在する」）、各セッションの最初のイベントのみにフィルターされます。<p>次の操作を行うには、このコンポーネントをデータビューに含める必要があります [計算指標](/help/components/calc-metrics/default-calcmetrics.md) Workspace で使用可能にするには： <ul><li>セッション開始率</li></p> |
| [!UICONTROL セッション終了] | 指標 | セッションの最後のイベントとなったイベントの数。「[!UICONTROL セッション開始]」と同様、これをフィルター定義で使用してフィルタリングし、各セッションの最後のイベントだけを残すこともできます。<p>次の操作を行うには、このコンポーネントをデータビューに含める必要があります [計算指標](/help/components/calc-metrics/default-calcmetrics.md) Workspace で使用可能にするには： <ul><li>セッション終了率</li></p> |
...
| [!UICONTROL 滞在時間 (秒)] | 指標 | あるディメンションについて、2 つの異なる値の間の時間を合計します。<p>次の操作を行うには、このコンポーネントをデータビューに含める必要があります [計算指標](/help/components/calc-metrics/default-calcmetrics.md) Workspace で使用可能にするには： <ul><li>個人別滞在時間</li><li>セッションごとに費やされた時間</li></p> |
