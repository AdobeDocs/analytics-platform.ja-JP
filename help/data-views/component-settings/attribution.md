...
アトリビューションを使用すると、成功イベントの貢献度をディメンション項目にどのように割り当てるかをカスタマイズできます。
...
一部のレポートでは、注文を有料検索に関連付けることが望まれるかもしれません。 他のレポートでは、注文を「ソーシャル」に関連付けることが望まれるかもしれません。 アトリビューションを使用すると、このレポートの側面を制御できます。

## コンポーネントのアトリビューションモデルの設定

特定のコンポーネントのデフォルトのアトリビューションモデルを変更するには、データビューでコンポーネントの設定を更新します。 これにより、Analysis Workspaceで使用されるときはいつでも、コンポーネントのアトリビューションモデルが上書きされます。

>[!NOTE]
>
>指標でデフォルト以外のアトリビューションモデルを有効にする場合は、次の点を考慮してください。
>
>* **単一ディメンションが&#x200B;*あるレポートで指標を使用する場合*:**&#x200B;指標のアトリビューションは、ディメンションで設定された配分モデルを上書きします。 例えば、「ファーストタッチ」アトリビューションを持つ指標は、「最新」ディメンション割り当てを上書きします。
>
>* **複数のディメンションを含むレポートで指標を使用する場合&#x200B;*複数のディメンション*:**&#x200B;指標のアトリビューションは、各ディメンションの配分モデルの上に適用されます。 例えば、「ファーストタッチ」アトリビューションを持つ指標は、「最新」ディメンション割り当ての上に適用されます。
>
...
1. コンポーネントを選択し、画面の右側にある「**[!UICONTROL アトリビューション]**」セクションを展開します。

   ![「アトリビューションを設定」オプションをハイライト表示するデータビューウィンドウ](../assets/attribution-settings.png)

1. [!UICONTROL **Set attribution**]&#x200B;を選択し、[attribution model](#attribution-models)、[container](#container)、[lookback](#lookback-window) ウィンドウを選択します。
