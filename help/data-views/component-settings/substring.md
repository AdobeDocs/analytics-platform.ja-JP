...
[!UICONTROL 部分文字列]は、ディメンションでのみ使用でき、適用対象となるデータに遡って適用されます。 セグメンテーションなどの分析作業に先駆けてデータを変換します。

## 左／右から

文字列の先頭または末尾に対する位置に基づいて、文字列の一部を取ります。 **[!UICONTROL 左から]**&#x200B;および&#x200B;**[!UICONTROL 右から]** メソッドには、2つのドロップダウンメニューが用意されています：**[!UICONTROL 開始]** （出力が開始される場所）と&#x200B;**[!UICONTROL 開始]** （出力が終了する場所）。

* **[!UICONTROL 文字列の開始]**： 文字列の開始。
* **[!UICONTROL 文字列の終了]**：文字列の終了。
* **[!UICONTROL 位置]**：メソッドに応じて左または右からの文字列の静的な数値。
* **[!UICONTROL 文字列]**：文字列の先頭または末尾を示す文字または文字のシーケンスに一致します。 このドロップダウンメニューには、追加のオプションも表示されます。
...
* **[!UICONTROL 長さ]**：出力の開始位置の後に含める文字数を指定する整数。 **[!UICONTROL から]** ドロップダウンメニューでのみ使用できます。
...
Customer Journey Analytics では、Perl 正規表現構文のサブセットを使用します。 入力が正規表現に一致せず、**[!UICONTROL 出力形式]**&#x200B;が空の場合、[値オプションなし](no-value-options.md)が適用されます。 次の式がサポートされています。
