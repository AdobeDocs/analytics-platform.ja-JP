...
この設定は通常、オブジェクト配列で使用され、製品検索方法、内部検索結果、内部プロモーションのインプレッション、コンテンツや製品レコメンデーションの測定に最も一般的に使用されます。 以前のバージョンのAdobe Analyticsでは、この概念はコンバージョン構文マーチャンダイジングと呼ばれていました。

## [!UICONTROL バインディング指標]

バインディング指標として機能する指標を選択するためのドロップダウントリガー。 有効なドロップダウンオプションには、データビューに含まれる指標が含まれます。

この設定は、連結Dimensionがコンポーネントよりもオブジェクト配列で低い場合にのみ表示されます。 イベントにバインディング指標が存在する場合、ディメンション値はイベントレベルのディメンションからバインディングディメンションの下位のスキーマレベルにコピーされます。 以前のバージョンのAdobe Analyticsでは、これと似ていますが、より限られた概念は、製品構文マーチャンダイジングと呼ばれていました。
