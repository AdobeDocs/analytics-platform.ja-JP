...
* **[!UICONTROL 最初の既知]** および **[!UICONTROL 最新の既知]**:（ 2022 年 1 月 20 日）これらの 2 つの配分モデルは、「入口」および「出口」ディメンションの使用例を満たします。 指定した永続性スコープ（ルックバックを含むセッション、ユーザーまたはカスタム期間）内のディメンションの最初または最後に観測された値を取得し、指定したスコープ内のすべてのイベントに適用します。 例：
