...
[!UICONTROL 永続性]はディメンションでのみ使用でき、適用されたデータに遡及的に適用されます。 セグメンテーションなどの分析作業に先駆けてデータを変換します。
...
* **[!UICONTROL 元の]**：有効期限の間、ディメンション内に存在するタイムスタンプによって元の値を保持します。 このディメンションに値がある場合、後続のイベントで別の値が表示されたときに上書きされません。 例えば、 [!UICONTROL オリジナル] の配分と [!UICONTROL セッション] の有効期限を含む次の表について考えてみましょう。
...
ディメンション値の永続性を別のディメンションのディメンション値にバインドできるドロップダウンメニュー。 有効なオプションには、データビュー内にある他のディメンションが含まれます。

バインディングディメンションを効果的に使用する方法の例については、[Customer Journey Analyticsでのバインディングディメンションと指標の使用](../../use-cases/data-views/binding-dimensions-metrics.md)を参照してください。
...
ドロップダウンメニュー。バインディングトリガーとして機能する指標を選択できます。 有効なオプションには、データビュー内にある指標が含まれます。
...
バインディング指標を効果的に使用する方法について詳しくは、[Customer Journey Analyticsでのバインディングディメンションと指標の使用](../../use-cases/data-views/binding-dimensions-metrics.md)の2番目の例を参照してください。
