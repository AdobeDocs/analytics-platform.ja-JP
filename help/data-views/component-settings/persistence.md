...
[!UICONTROL 永続性] は、ディメンションに対してのみ使用でき、適用対象となるデータに遡って適用されます。セグメント化や他の分析操作が適用される前に発生する即時のデータ変換です。
