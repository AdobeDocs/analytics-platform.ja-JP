...
形式を使用すると、レポートで使用する際に、特定のコンポーネントがどのように表示されるかを指定できます。

## コンポーネントのフォーマット設定の設定

特定のコンポーネントの表示方法は、その書式設定を調整することによって決定できます。
...
1. 次の情報を指定します。
   ...
   | 設定 | 説明 |
   | --- | --- |
   | **[!UICONTROL 形式]** | コンポーネントの書式設定を10進数、時間、パーセントまたは通貨として指定できます。 |
   | **[!UICONTROL 小数点以下の桁数]** | 整数スキーマのデータタイプには表示されません。 コンポーネントを表示する小数点以下桁の数を指定できます。 |
   ...
   | **[!UICONTROL 通貨]** | コンポーネントに表示する通貨を決定できます。 <p>様々な通貨で取引が発生するグローバルデータを分析する場合は、[通貨換算の使用](#use-currency-conversion)を参照してください。</p> |
   | **[!UICONTROL 上昇傾向を次の形式で表示]** | このコンポーネントの上昇傾向が良い（緑）か悪い（赤）かを指定できます。 |
