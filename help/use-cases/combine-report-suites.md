...
1. [新しいスキーマを作成](https://experienceleague.adobe.com/docs/experience-platform/xdm/ui/overview.htm?lang=ja) AEP 内（と呼びます） **統合スキーマ** （この例では）。 スキーマに次のフィールドグループを追加します。
   ...
   のソース接続データフローを作成する場合 **レポートスイート A**&#x200B;を選択します。 **統合スキーマ** をデータフローで使用します。

1. 次のように、カスタムマッピングを追加します。
   ...
   >[!NOTE]
   >
   >宛先フィールドの XDM パスは、カスタムフィールドグループの設定方法に応じて異なります。

1. のソース接続データフローを作成する場合 **レポートスイート B**&#x200B;を再度選択します。 **統合スキーマ** をデータフローで使用します。

   このワークフローでは、2 つのフィールドに記述子名の競合があることが示されています。 これは、eVar1 とeVar2 の記述子がレポートスイート B ではレポートスイート A では記述子と異なるためです。しかし、既にこれを把握しているので、競合を無視して、次のようにカスタムマッピングを使用できます。
...
1. の作成 **グローバル表示** CJA でのデータ表示。

   元のeVarフィールドを無視し、[ 統合フィールド ] フィールドグループのフィールドのみを含めます。
...
1. AEP でスキーマを作成します。 スキーマに次のフィールドグループを追加します。
...
1. のデータフローを作成する場合 **データセット A**&#x200B;を選択します。 **拡張スキーマ** をスキーマとして使用します。
...
1. のデータフローを作成する場合 **データセット B**&#x200B;を再度選択します。 **拡張スキーマ** をスキーマとして使用します。
