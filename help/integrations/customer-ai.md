...
（この例はプロファイルデータセットです。同じスキーマオブジェクトセットは、Customer Journey Analytics が取得するエクスペリエンスイベントデータセットの一部になります。 Experience Event データセットには、スコア日付としてタイムスタンプが含まれます）。 このモデルでスコアを付けた顧客には、スコア、scoreDateなどが関連付けられます。
