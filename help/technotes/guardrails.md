...
| 更新オーディエンスの数 | 75 - 150 | システム強制ガードレール | スケジュールされた頻度で[&#128279;](/help/components/audiences/publish.md#audience-builder)を更新するように設定されている オーディエンスの最大数（1回限りの更新はこの制限にカウントされません）。 値はCustomer Journey Analytics パッケージによって異なります（商品説明を参照）。 |
...
| テーブルあたりのディメンション | 10 | システム強制ガードレール | テーブルあたりの最大ディメンション数。 |
| テーブルあたりの指標 | 10 | システム強制ガードレール | テーブルあたりの指標の最大数。 |
...
| オーディエンスの更新頻度 | 4時間ごとに、毎日、毎週、毎月更新する。 |
