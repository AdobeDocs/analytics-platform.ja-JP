...
| 名前 | 値 | 上限のタイプ | 説明 |
|---|--:|---|---|
| Try Again Timeout | 90 | システム強制ガードレール | （他のリクエストの同時実行が原因で）結果を返すのにリクエストに時間がかかりすぎるというレポートエンジンが応答を返すまでの最大秒数。もう一度リクエストすることは可能です。 |
...
| 1 年あたりの接続あたりの行数 | 割り当てられたデータセンターによって異なる（詳しくは説明を参照） | パフォーマンスガードレール | 接続における 1 年あたりの推奨される行数の制限。 以下に示す制限は 2026 年 1 月からのおおよその時点のもので、時間の経過と共に増加し、接続あたりの行数を増やすことができます。 これらの上限を超える場合は、Adobe アカウントチームに連絡して、代替設定について相談してください。 <p>行の制限は、次のように、割り当てられたデータセンターによって異なります。</p><ul><li>**米国Azure （米国のお客様のデフォルト）**：約 5,000 億（月に約 420 億）</li><li>**米国AWS（米国のお客様のご要望で提供）**：約 300 億（月あたり約 25 億人）</li><li>**アムステルダム**：約 2 千億（月あたり約 165 億）</li><li>**その他のデータセンターすべて**:250 億（1 か月あたり約 20 億）</li></ul></p><p>Experience Platformを最初に購入する際に、Experience Platform データセンターを選択します。 この決定は、通常、データ主権と常駐サービスの要件に基づいています。 データセンターを選ぶ際には、（Customer Journey Analyticsの行数だけでなく）AEPとアプリのすべてのユースケースを考慮する必要があります。</p><p>割り当てられているデータセンターを表示する方法については、[Customer Journey Analyticsのホスティングの場所を参照してください ](/help/technotes/data-centers.md)</p> |
