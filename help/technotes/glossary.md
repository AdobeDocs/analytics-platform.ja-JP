...
| Profile データセット | 顧客属性 | エンタープライズ顧客データを顧客関係管理（CRM）データベースに取り込んでいる場合は、そのデータを Adobe Experience Platform のプロファイルデータセットにアップロードできます。 Customer Journey Analytics でそのデータセットへの接続を作成し、データビューを作成したら、ワークスペースでそのデータを利用できます。 |
| CX Enterprise （Experience Cloud）組織 | ログイン会社 | [組織とアカウントのリンク](https://experienceleague.adobe.com/docs/core-services/interface/manage-users-and-products/organizations.html?lang=ja#topic_C31CB834F109465A82ED57FF0563B3F1)を参照してください。 |
...
| セグメント | セグメント | セグメントは「フィルター」でした。 「セグメント」に名前が変更されました。 |
...
## Adobe Experience Platform 用語集

Adobe Experience Platform は、企業全体のデータとコンテンツを標準化し、リアルタイムの消費者プロファイルを強化し、データサイエンスを可能にし、コンテンツベロシティを向上させて、カスタマージャーニー全体にわたるエクスペリエンスのパーソナライゼーションを推進します。
