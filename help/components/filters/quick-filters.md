...
| コンポーネント (Dimension/指標/日付範囲 ) | コンポーネント（ディメンション、指標、日付範囲、ディメンション値）を追加して、最大 3 つのルールを定義できます。 適切なコンポーネントを見つける方法は 3 つあります。<ul><li>入力を開始すると、[!UICONTROL  クイックフィルター ] ビルダーによって適切なコンポーネントが自動的に検索されます。</li><li>ドロップダウンリストを使用して、コンポーネントを検索します。</li><li>左パネルからコンポーネントをドラッグ＆ドロップします。</li></ul> |
| 演算子 | 標準演算子と [!UICONTROL  個別カウント ] 演算子を検索するには、ドロップダウンメニューを使用します。 [ フィルター演算子 ](operators.md) を参照してください。 |
| プラス記号 (+) | 別のルールの追加 |
