...
自分が作成した書き出しのみを表示できます。 管理者は、「すべてのユーザーの書き出しを表示 **[!UICONTROL オプションを有効にすることで]** すべての書き出しを表示できます。
...
1. 「[!UICONTROL **書き出し**]」タブを選択します。
   ...
   ![ 書き出しを管理ページ ](assets/exports-manage.png)
...
1. **フィルター** アイコン ![ フィルターアイコン ](/help/assets/icons/Filter.svg) を選択します。
   ...
   | [!UICONTROL **ステータス**] | エクスポートのステータス。 次のステータスを表示できます。 <ul><li>[!UICONTROL **アクティブ**]：スケジュールされた書き出しがまだ期限切れになっていないか、1 回限りの書き出しがまだ完了していないことを示します。 </li><li>[!UICONTROL **完了**]：書き出しが正常に書き出されたことを示します。 スケジュールされた書き出しの場合は、スケジュールの有効期限が切れたことを示します。</li><li>[!UICONTROL **失敗**]<p>次の状況では、書き出しに失敗する場合があります。 [!UICONTROL **失敗**] ステータスの上にマウスポインターを置くと、失敗の詳細が表示されます。 <ul><li>スケジュールされた書き出しの有効期限</li><li>スケジュールされた書き出しの行制限に達しました </li></ul><li>[!UICONTROL **期限切れ**]：書き出しの有効期限が切れたことを示します。</li></ul> |
   | [!UICONTROL **作成者**] | 書き出しを作成したユーザー。<p>このオプションは、「すべてのユーザーの書き出しを表示 **[!UICONTROL オプションが有効になっている場合に]** 管理者のみが使用できます。 |
...
1. 使用可能なオプションを更新します。 各オプションについて詳しくは、[Analysis Workspace レポートのクラウドへの書き出し ](/help/analysis-workspace/export/export-cloud.md#export-full-tables) の [Customer Journey Analyticsからの完全なテーブルの書き出し ](/help/analysis-workspace/export/export-cloud.md) を参照してください。

## 書き出しの更新

有効期限が切れる前または後に、1 つ以上のスケジュールされた書き出しを更新できます。 書き出しは、書き出しを更新した日から 1 年間更新されます。

1. Customer Journey Analyticsで、[!UICONTROL **コンポーネント**]/[!UICONTROL **書き出し**] を選択します。

1. [!UICONTROL **書き出し**] タブで、更新する 1 つ以上の書き出しの横にあるチェックボックスをオンにします。

1. 「[!UICONTROL **更新**]」を選択します。

   [!UICONTROL **完全なテーブルを書き出し**] ダイアログが表示されます。<!--check process from here. -->
...
タグを書き出しに適用すると、それらのタグを [!UICONTROL  書き出し ] ページの [!UICONTROL  タグ ] 列で表示できます。 詳しくは、[ 列の設定 ](#configure-columns-on-the-exports-page) を参照してください。
...
書き出しは、書き出しページから削除できます。 書き出しを削除すると、書き出しページから削除されます。 スケジュールされた書き出しは削除されるとキャンセルされ、送信されなくなります。
...
1. 「[!UICONTROL **書き出し**]」タブで、**書き出し** ページの右上にある「![ テーブルをカスタマイズ ](assets/customize-table-icon.png) アイコン [!UICONTROL  テーブルをカスタマイズ ] を選択します。
   ...
   | 頻度 | 書き出しを送信する頻度。 使用できるオプションは、[!UICONTROL 1 回 ]、[!UICONTROL  毎日 ]、[!UICONTROL  毎週 ]、[!UICONTROL  毎月 ] 曜日、[!UICONTROL  毎月 ]、[!UICONTROL  毎年 ]、および [!UICONTROL  毎年の特定の日付 ] です。 |
   | 送信時間 | エクスポートが送信された時刻。 |
...
[ クラウドへの完全なテーブルの書き出し ](/help/analysis-workspace/export/export-cloud.md) の説明に従ってAnalysis Workspaceから、または、この節の説明に従って書き出しページから、書き出しを作成できます。

「書き出し」ページから書き出しの作成を開始するには、次の手順を実行します。

1. Customer Journey Analyticsで、[!UICONTROL **コンポーネント**]/[!UICONTROL **書き出し**] を選択します。

1. 「[!UICONTROL **書き出し**]」タブで、「**[!UICONTROL 書き出しを追加]**」を選択します。

1. 使用可能なフィールドに入力して、書き出しを作成します。 各フィールドの情報、およびコンポーネント、計算指標関数、サポートされているその他の機能に関する情報については、[ クラウドへの完全なテーブルの書き出し ](/help/analysis-workspace/export/export-cloud.md) を参照してください。
