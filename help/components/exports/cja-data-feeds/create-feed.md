...
>title="問題、完了時、期限切れ時に通知"
>abstract="データフィードの完了時、期限切れ時、問題の発生時に通知を配信するメールアドレスを 1 つ以上指定します。 複数のメールアドレスをコンマで区切ります。"
...
>title="遅延処理"
>abstract="データフィードファイルを処理する前に遅延イベントを待つ時間。 遅延処理期間中に発生した遅延ヒットは、データフィードに含まれます。 <p>遅延処理は、オフラインデバイスがオンラインでデータを送信する機会をモバイル実装に提供する場合や、以前に処理されたファイルを管理する際に組織のサーバーサイドのプロセスに対応する場合など、様々な理由で役立ちます。</p><p>セッションを含めるには、遅延処理のカットオフの後にセッションを開始する必要があります。カットオフの前に開始し、遅延処理内で終了するセッションは含まれません。</p><p>Customer Journey Analytics では、フィードに対して遅れて到着するイベントの通常の所要時間に基づいて最適な遅延を動的に決定しますが、2 時間、3 時間、4 時間、8 時間の遅延を手動で設定することもできます。</p>"
...
1. 「[!UICONTROL **配信**]」タブの「[!UICONTROL **スケジュール**]」セクションで、作成するフィードのタイプ（ライブまたはバックフィル）を選択し、レポートウィンドウ、頻度、その他の設定オプションを指定します。
   ...
   | [!UICONTROL **処理遅延**] | データフィードファイルを処理するまでの待機時間を選択します。 デフォルトは2時間です。 処理遅延の間に発生した遅延イベントは、データフィードに含まれます。 <p>遅延処理は、オフラインデバイスがオンラインでデータを送信する機会をモバイル実装に提供する場合や、以前に処理されたファイルを管理する際に組織のサーバーサイドのプロセスに対応する場合など、様々な理由で役立ちます。 </p><p>セッションを含めるには、遅延処理のカットオフの後にセッションを開始する必要があります。カットオフの前に開始し、遅延処理内で終了するセッションは含まれません。</p><p>Customer Journey Analytics では、フィードに対して遅れて到着するイベントの通常の所要時間に基づいて最適な遅延を動的に決定しますが、2 時間、3 時間、4 時間、8 時間の遅延を手動で設定することもできます。</p> |
...
>title="ルックバック日付範囲"
>abstract="各配信を処理する際に、Customer Journey Analytics がルックバックする日付範囲を制御します。<p>頻度ウィンドウ（時間または日）では、データフィードに含まれるイベントを決定します。一方、**ルックバック日付範囲**&#x200B;では、これらのイベントを正しく分類するために必要な履歴コンテキストを提供します。</p><p>セグメントの選定、ディメンションの永続性、セッションの計算、派生フィールド変換はすべて、含まれるイベントに影響を与える場合があります。</p><p>ルックバックが長いほど精度が向上し、ルックバックが短いほどパフォーマンスが向上します。</p>"
