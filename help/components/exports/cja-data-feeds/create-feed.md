...
1. 「[!UICONTROL **配信**]」タブの「[!UICONTROL **スケジュール**]」セクションで、作成するフィードのタイプ（ライブまたはバックフィル）を選択し、レポートウィンドウ、頻度、その他の設定オプションを指定します。
   ...
   | [!UICONTROL **ルックバック日付範囲**] | データフィード配信を処理する際に、Customer Journey Analytics がルックバックする日付範囲を制御します。 デフォルトは30日です。 <p>ルックバック日付範囲は、セグメントの選定、セッション計算、派生フィールド変換、ディメンションの永続性に影響します。 <p>このオプションを設定する前に、次の節「[ ルックバック日付範囲について](#understand-the-lookback-date-range)」で説明されている詳細と例を参照してください。</p> |
...
>title="ルックバック日付範囲"
>abstract="データフィード配信を処理する際に、Customer Journey Analytics がルックバックする日付範囲を制御します。 この設定は、Analysis Workspace レポートの日付範囲と似ていますが、重要な違いがあります。<ul><li>イベントが、ルックバック日付範囲内ではなく頻度ウィンドウ内に収まるタイムスタンプがある場合、イベントはデータフィードに含まれます。 （Analysis Workspaceでは、タイムスタンプがレポート日付範囲内にある場合、イベントはレポートに含まれます）。</li><li>タイムスタンプがルックバック日付範囲内（ただし、頻度ウィンドウ外）にあるイベントは、セグメントの選定、セッション計算、派生フィールド変換、ディメンションの永続性を通じて、フィードに表示されるデータに影響を与える可能性があります。</li><p>通常、ルックバック日付範囲が長いほどイベントの精度が高くなり、範囲が短いほど配信パフォーマンスが向上します。</p>"

<!-- markdownlint-enable MD034 -->



ルックバック日付範囲は、データフィード配信を処理する際にCustomer Journey Analyticsがどの程度前を見るかを制御します。 デフォルトは30日です。

このオプションを設定する際には、次の重要な概念を考慮してください。

* 通常、ルックバック日付範囲が長いほどデータの精度が高くなり、範囲が短いほど配信パフォーマンスが向上します。
* データフィードのルックバック日付範囲は、Analysis Workspaceのレポート日付範囲と似ていますが、[主な違い](/help/components/exports/cja-data-feeds/df-comparison-workspace.md#differences)があります。 このような違いにより、Workspace レポートとデータフィードの配信の間でデータの相違が生じる可能性があります。
* ルックバック日付範囲は、データフィード出力に含めるイベントの時間枠を定義する頻度ウィンドウ（時間または日）を変更しません。
* ルックバック日付範囲に入るデータは、以下のセクションで説明する要因に応じて、データフィード（頻度ウィンドウ）に含まれるものに影響を与える可能性があります。
...
セッションの境界は、ルックバック日付範囲内のデータを使用して計算されます。 セッション IDとは何か？ セッション IDに影響を与える可能性はありますか？ セッションベースの永続性など、多くのことに影響を与える可能性があります。

### 派生フィールド変換

コンテナを参照する派生フィールド関数は、データフィードの書き出しでルックバック日付範囲を使用します。 派生フィールドにはどのような日付機能がありますか？ どのように適用されるのかはわかりません。
