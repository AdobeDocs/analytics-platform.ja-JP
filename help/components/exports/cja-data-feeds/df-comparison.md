...
* 派生フィールド
  ...
  派生フィールドコンポーネントをデータフィードに含める。

* ステッチ

  クロスデバイス ID解決を有効にし、デバイス間のイベントを1人のユーザーにリンクします。

* 構造化されたデータ像

  配信ファイルでデータフィードを構築する場合に、構造化データを使用します。 Adobe Analyticsのデータフィードには文字列が使用されます。

* Analysis Workspaceに一致するディメンションと指標を含むコンポーネントパネル

  は、データビューで使用可能なディメンションと指標を使用します。 Adobe Analyticsでは、事前に定義されたフィールドと列のリストが使用されます。

* データビューに適用されたセグメントは、データフィードで自動的に継承されます

* セグメントは、データビューですでに適用されているセグメントに加えて、データフィードに直接適用できます

* フィードがデータ ビューのタイム ゾーン <!-- how did it work in AA? -->と一致しています

* 寄木細工の配送

  複雑なネスト化データと構造化データをネイティブにサポートする最新のParquet ファイルを出力します。 商品リストは、構造化配列やネストされたオブジェクトとして表されます。

* Hive スタイルのパス

* データビュー内のコンポーネントに加えられた変更は、データフィードに反映されます
