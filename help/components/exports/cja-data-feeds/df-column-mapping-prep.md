...
* **[データフィードスキーマアーキテクチャ](#schema-architecture)**: Adobe Analytics データフィード列はAnalytics変数から派生し、Customer Journey Analytics データフィード列はデータビュー設定から派生します。
...
* **組織に適用されるフィールドを特定**：すべてのAdobe Analyticsのお客様が使用可能な列をすべて書き出すわけではありませんが、多くのお客様は、実際に使用する以上のものを書き出します。
  ...
  Customer Journey Analyticsからのデータフィードの書き出しを開始する前に、まず、組織が現在使用しているAdobe Analytics データフィード列を特定し、それらのコンポーネントがCustomer Journey Analytics データビュー設定に存在することを確認する必要があります。 この情報を収集するには、Adobe Analyticsのデータフィードのコンテンツを利用する組織全体のチームまたは個人にお問い合わせください。
