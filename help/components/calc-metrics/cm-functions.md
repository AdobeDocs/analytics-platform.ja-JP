...
**計算指標ビルダー**:**日** または *製品* で分類する場合に、*売上高* または *セッション* などの指標に *列の最大値* を適用します。 この関数は、各行のその列の最大値を返します。
...
**計算指標ビルダー**:**製品** または *日* で分類しながら、*売上高* または *セッション* などの指標に *列合計* を適用します。 この関数は、各行のその列に含まれるすべての値の合計を返します。
...
**計算指標ビルダー**:**カウント** を *セッション数* や *売上高* などの指標に適用して、現在の分類または日付範囲に含まれる行の合計数（またはデータポイント）を返します。
...
[!BADGE Row]{type="Neutral"} *e* を指定された数値の累乗で返します。 定数 *e* は、自然対数の底である 2.71828182845904 に等しくなります。 EXPONENT（指数）は LN（数の自然対数）の逆関数です。
...
**計算指標ビルダー内**：指標で **指数** を使用します。 例：**Exponent** （*Sessions*）は *e* を *Sessions* 指標の累乗に上げます。
...
**計算指標ビルダー**:**売上高** または *セッション数* などの指標に *平均* を適用して、選択した分類または日付範囲のすべてのデータポイントの平均値を返します。
...
**計算指標ビルダー**:**パーセンタイル** を *売上高* や *セッション数* などの指標に適用し、目的のパーセンタイル値（例：**パーセンタイル** （*売上高*、90）を指定します）。 その結果、データポイントの 90% が低下するしきい値が示されます。
...
**計算指標ビルダー**:**四分位数** を *売上高* や *セッション数* などの指標に適用し、返す四分位数を指定します（例：**四分位数** （*売上高*、3）。これにより、3 番目の四分位数のしきい値、つまり上位 25% を見つけることができます）。
...
**ユースケース**：特定の日またはセグメントに最大の値を持つ指標（例：*売上高*、*注文件数*、*セッション*）の決定など、1 行のすべての指標の中で最も高い値を特定します。 これは、データの各行でどの指標リードをハイライト表示するのに役立ちます。

**計算指標ビルダー**：計算指標に複数の指標が含まれる場合に **行の最大値** を適用します。 例：**Row Maximum** （*Revenue*, *Orders*, *Sessions*）は、各行のこれらの指標の中で最大値を返します。
...
**ユースケース**：特定の日またはセグメントに最小値を持つ指標（例：*売上高*、*注文*、*セッション*）を検索するなど、1 行のすべての指標の中で最小値を特定します。 これにより、データの各行で最もパフォーマンスの低い指標を見つけることができます。

**計算指標ビルダー**：複数の指標を比較する際に **行最小値** を適用します。 例：**Row Minimum** （*Revenue*, *Orders*, *Sessions*）は、各行のこれらの指標の中の最小値を返します。
...
**ユースケース**:1 行に複数の指標の値をまとめたものです。例えば、*売上高* と *税金* を合計してトランザクション合計値を計算したり、様々なソースの *セッション* を組み合わせたりします。 これにより、関連指標を 1 つの合計に統合できます。
...
**計算指標ビルダー**:**標準偏差** を *売上高* または *セッション* などの指標に適用して、選択した分類または日付範囲内の値の配分を計算します。 例：**Standard Deviation** （*Revenue*）は、毎日の売上高が平均からどの程度逸脱しているかを示します。
