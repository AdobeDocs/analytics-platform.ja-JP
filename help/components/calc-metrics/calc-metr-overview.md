...
* （高度な計算指標のみ）指標でフィルタリングする。例えば、初回セッションの訪問者数を含む、「新規人物」の指標を作成できます。
