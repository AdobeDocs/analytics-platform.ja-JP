...
信頼性は、特定のバリアントがコントロールバリアントと同じであるという証拠がどの程度あるかを示す確率的な尺度です。 信頼性が高いほど、コントロールバリアントおよびコントロールバリアント以外のパフォーマンスが等しいという仮定に対する証拠が少ないことを示します。
...
[!BADGE 行]{type="Neutral"}指数回帰：Y = b * exp （aX）。 Y を返します。
...
| value_if_false | logical_test引数がFALSEと評価された場合に返す値。 （含まれない場合、この引数のデフォルト値は 0 です）。 |
...
![効果](/help/assets/icons/Effect.svg) **[!UICONTROL LIFT(normalizing-container, success-metric, control)]**
...
[!BADGE 表]{type="Neutral"}線形回帰：Y = a X + b. b を返します。
...
[!BADGE 表]{type="Neutral"}線形回帰：Y = a X + b. a を返します。
...
[!BADGE 行]{type="Neutral"} ログ回帰：Y = a ln （X） + b. Y を返します。
...
[!BADGE 表]{type="Neutral"}電力回帰：Y = b X ^ a. 相関係数を返します。
...
[!BADGE 表]{type="Neutral"}電力回帰：Y = b X ^ a. b を返します。
...
[!BADGE 行]{type="Neutral"}電力回帰：Y = b X ^ a. Y を返します。
...
[!BADGE 表]{type="Neutral"}電力回帰：Y = b X ^ a. a を返します。
