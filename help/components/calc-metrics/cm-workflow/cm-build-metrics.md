...
1. 計算指標の定義が正しいかどうかを確認するには、計算指標の結果の絶えず更新される&#x200B;**[!UICONTROL プレビュー]**&#x200B;を使用します。**[!UICONTROL プレビュー]**&#x200B;は過去 90 日間をカバーしており、計算指標の定義を継続的に評価します。
   ...
   **[!UICONTROL 製品の互換性]** は、計算指標を実験やテーブル全体の書き出しに使用できるかどうかを示します。 使用可能な値：
   * **[!UICONTROL Customer Journey Analytics のすべての場所]**：計算指標は、Adobe Customer Journey Analytics 全体で使用できます。
   * **[!UICONTROL 次のバージョンと互換性がありません：]**
      * **[!UICONTROL 実験]**：計算指標は、実験パネルを除くすべてのCustomer Journey Analyticsで使用できます。
      * **[!UICONTROL 完全テーブルの書き出し]**:Workspaceから完全なテーブルを書き出す場合を除き、計算指標はCustomer Journey Analytics全体で使用できます。 すべてのテーブルを書き出す場合、すべての関数がサポートされているわけではありません。 すべてのテーブルを書き出す際に計算指標を含める必要がある場合は、サポートされている関数を使用します。 詳しくは、[&#x200B; クラウドへの完全なテーブルの書き出し &#x200B;](/help/analysis-workspace/export/export-cloud.md#unsupported-calculated-metric-functions) の「[&#x200B; サポートされていない計算指標関数 &#x200B;](/help/analysis-workspace/export/export-cloud.md) を参照してください。
