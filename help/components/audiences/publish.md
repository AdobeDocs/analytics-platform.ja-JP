...
1. オーディエンスを構築します。
   ...
   | [!UICONTROL 更新頻度] | オーディエンスを更新する頻度。<ul><li>更新を必要としない 1 回限りのオーディエンス（デフォルト）の作成を選択できます。これは、特定の 1 回限りのキャンペーンなどに役立ちます。</li><li>その他の更新間隔を選択できます。すべての更新頻度に対して、CJA 使用権限に応じて、75 または 150 のオーディエンスの制限があります。 その他の間隔の場合は、オーディエンスの最大数はありません。</li></ul> |
...
1. データのプレビューを解釈します。
   ...
   | [!UICONTROL 人物合計] | このオーディエンスの合計人数の概要。2 千万人もの人々が集まります オーディエンスのユーザーが 2,000 万人を超える場合は、オーディエンスのサイズを小さくしてから公開する必要があります。 |
   | [!UICONTROL オーディエンスサイズの制限] | このオーディエンスが 2,000 万件の制限からどの程度離れているかを表示します。 |
...
CJA は、公開済みのオーディエンスからすべての名前空間と ID の組み合わせを取得し、それらをリアルタイム顧客プロファイル（RTCP）にストリーミングするようになりました。CJA は、接続設定時に人物 ID として選択されたものに対して設定されたプライマリ ID を使用して、オーディエンスを Experience Platform に送信します。
...
## CJA でユーザーがオーディエンスのメンバーでなくなった場合 {#no-member}

この場合、exit イベントが CJA から Experience Platform に送信されます。

## CJA でオーディエンスを削除するとどうなりますか。 {#delete}

CJA オーディエンスが削除されると、そのオーディエンスはExperience PlatformUI に表示されなくなります。 ただし、そのオーディエンスに関連付けられたプロファイルは、実際には Platform で削除されません。
