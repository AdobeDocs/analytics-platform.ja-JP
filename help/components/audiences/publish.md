...
1. データのプレビューを解釈します。
   ...
   | [!UICONTROL オーディエンス再来訪の見込み] | この設定は、サイト、モバイルアプリまたは他のチャネル（つまり、このデータセットに再び表示される）に戻ってきた、このオーディエンスの顧客をリターゲティングする場合に役立ちます。 <p>ここでは、再来訪の可能性がある顧客の推定数に対して期間（7 日間、次の 2 週間、次の月）を選択できます。 |
...
![この節で説明した、オーディエンス公開の待ち時間。](assets/latency-diagram.png)
