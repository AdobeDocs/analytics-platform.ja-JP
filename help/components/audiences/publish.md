...
1. オーディエンスを構築します。
   ...
   | [!UICONTROL 更新頻度] | オーディエンスを更新する頻度。<ul><li>更新を必要としない 1 回限りのオーディエンス（デフォルト）の作成を選択できます。これは、特定の 1 回限りのキャンペーンなどに役立ちます。</li><li>その他の更新間隔を選択できます。すべての更新頻度に対して、CJA 使用権限に応じて、75 ～ 150 のオーディエンスの制限があります。</li></ul> |
...
パイプラインを介してデータを RTCP にストリーミングし、このデータもデータレイクのシステムデータセットに収集されます。

### CJA はどの ID を送信しますか。

接続設定で使用された ID/名前空間のペア。 特に、ユーザーが「ユーザー ID」として使用するフィールドを選択する手順です。
