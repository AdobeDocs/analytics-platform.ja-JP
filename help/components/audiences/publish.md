...
| 2 | データのExperience Platformから CJA への取り込み | 最大 60 分 |
| 3 | オーディエンスをリアルタイム顧客プロファイルに公開する（ストリーミングセグメントの自動作成を含む）。また、セグメントがデータを受け取る準備ができる状態になることも含む )。 | 約 60 分 |
| 4 | オーディエンスの更新頻度 | <ul><li>1 回の更新（5 分未満の遅延）</li><li>4 時間ごと、日別、週別、月別に更新します（遅延は更新率と連携します）。 |
...
+++**CJA は、オーディエンスデータをパイプラインイベント、またはデータレイクに送られるフラットファイルのどちらを介して送信されますか。**
