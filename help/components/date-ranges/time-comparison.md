...
Analysis Workspaceの日付比較では、日付範囲を含む任意の列を使用して、前年比、四半期比、月比などの共通の日付比較を作成できます。

## 期間を比較

分析にはコンテキストが必要であり、前期間がその情報を提供することがよくあります。 例えば、質問&#x200B;*昨年の現在と比較して、現在どの程度改善または悪化していますか？* 非常に重要です。 日付の比較では自動的に&#x200B;*差異*&#x200B;列が含まれ、指定した期間と比較した変更の割合が示されます。
...
1. 比較の時間枠を決定し、それがローリングまたは固定時間比較であるかどうかを判断するには、パネルまたは列で期間を設定します。

   周期的な時間比較を作成するには、パネルまたは列の日付範囲を周期的な日付範囲（**[!UICONTROL 過去 7 日間]**、**[!UICONTROL 過去 30 日間]**&#x200B;など）に設定します。

   固定時間比較を作成するには、パネルまたは列の日付範囲をカスタム日付範囲に設定します。
...
1. テーブルの日付範囲の設定方法に応じて、比較には次のオプションがあります。
...
テーブルの各列に期間を追加できるようになりました。 これにより、カレンダーが設定されている期間とは異なる期間を追加できます。
...
1. テーブルの日付範囲の設定方法に応じて、比較には次のオプションがあります。
   ...
   | オプション | 説明 |
   |---|---|
   | **[!UICONTROL この日付範囲の前の *x* 週／月／四半期／年]** | この日付範囲の直前に週/月などを含む列を追加します。 |
...
* この設定は、すべての新規プロジェクトに対してデフォルトで有効になっています。

* この設定は、テーブル全体に適用されます。 例えば、テーブル内の分類に対してこの設定を変更すると、設定はテーブル全体に適用されます。

* この設定を有効にすると、すべての列セルの右上隅に小さな日付ラベルが表示され、セルに適切な日付（および適切な場合は時間）の注釈が付けられます。
