...
1. テーブルの列を右クリックし、「**[!UICONTROL 期間列を追加]**」を選択します。![ 期間列を追加」がハイライト表示されたテーブル列リスト ](assets/add-time-period-column.png) 選択します
...
![ テーブル設定のポップアップが表示されたフリーフォームテーブル。各列の日付を、選択した同じ行のすべての開始日に整列します。](assets/date-comparison-setting.png)
