...
順次セグメントを作成する際の基本は、[ セグメントビルダー ](seg-builder.md) を使用して通常のセグメントを作成する場合と同じです。 [ 定義ビルダー ](seg-builder.md#definition-builder) を使用して、セグメント定義を作成できます。 この構成では、コンポーネント、コンテナ、演算子、ロジックを使用します。 通常のセグメントは、メイン定義または [ 定義ビルダー ](seg-builder.md#definition-builder) 内で使用する任意のコンテナで **[!UICONTROL Then]** 演算子を選択するとすぐに、順次セグメントになります。
...
### [!UICONTROL Event]、&lbrace;Session および [!UICONTROL 4&rbrace;Dimension] の制約
...
あるページを訪問した人、別のページを訪問しなかった人、さらに別のページを訪問した人を特定します。 ![ 設定 ](/help/assets/icons/Setting.svg) 除外  を使用してコンテナを除外  ます。 除外されたコンテナは、左側の薄い赤いバーで識別されます。
