...
description: フリーフォームテーブル、棒グラフ、散布図などのビジュアライゼーションを使用してContent Analyticsについてレポートする方法を説明します。
...
プロジェクトで使用するContent Analytics固有のディメンションに基づいて、次のビジュアライゼーションにサムネールが表示されます。
...
>[!AVAILABILITY]
>
>この節で説明する棒グラフと散布図のビジュアライゼーションの機能は、リリースの限定的テスト段階にあり、お使いの環境ではまだ使用できない可能性があります。 機能が一般公開されたら、このメモは削除されます。Customer Journey Analytics リリースプロセスについて詳しくは、[Customer Journey Analytics 機能リリース](/help/release-notes/releases.md)を参照してください。
>
...
* **アセットはサイトのどこに表示されますか？**
最も多く表示されたアセットがサイト上で表示される場所の詳細を示す 1 つのフリーフォームテーブルで構成されるパネル。 この分析により、最もパフォーマンスの高いアセットから最もメリットを得るページやセクション、およびプレースメントを最適化する場所を理解できます。
