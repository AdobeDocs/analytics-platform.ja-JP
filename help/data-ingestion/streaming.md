...
>[!NOTE]
>
>このクイックスタートガイドは、ストリーミングデータをAdobe Experience Platformに取り込み、Customer Journey Analyticsで使用する方法に関する簡略化されたガイドです。 参照する際には、追加情報を調べることを強くお勧めします。
...
このデータを作成したデータセットにストリーミングするには、そのデータを送信するストリーミングエンドポイントを定義する必要があります。 ストリーミングエンドポイントは、HTTP API ソースコネクタを使用して作成されます。
...
- ストリーミングコネクタの設定の一環としてデータセットを作成する方法。
