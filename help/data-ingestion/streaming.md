...
>[!NOTE]
>
>このクイックスタートガイドは、ストリーミングデータをAdobe Experience Platformに取り込み、Customer Journey Analyticsで使用する方法を簡単に説明するガイドです。 参照する際には、追加情報を調べることを強くお勧めします。
...
このクイックスタートでは、ロイヤルティ ID、ロイヤルティポイント、ロイヤルティステータスなど、一部のロイヤルティデータを収集します。最初に、このデータをモデル化するスキーマを定義する必要があります。
...
このデータを作成したデータセットにストリーミングするには、そのデータの送信先のストリーミングエンドポイントを定義する必要があります。 ストリーミングエンドポイントは、HTTP API ソースコネクタを使用して作成されます。
