...
description: Customer Journey Analyticsでのストリーミングデータの取り込みと使用方法について説明する
...
このクイックスタートガイドでは、ストリーミングデータをAdobe Experience Platformに取り込んで、そのデータをCustomer Journey Analyticsで使用する方法について説明します。

これをおこなうには、次の操作が必要です。

- **スキーマとデータセットの設定** Adobe Experience Platformで、収集するデータのモデル（スキーマ）と、実際にデータ（データセット）を収集する場所を定義します。

- **HTTP API ソースコネクタの使用** を使用すると、Adobe Experience Platformで設定したデータセットに簡単にデータをストリーミングできます。
...
>[!NOTE]
>
>これは、ストリーミングデータをAdobe Experience Platformに取り込み、Customer Journey Analyticsで使用する方法を簡単に説明するガイドです。  を参照する際には、追加情報を調べることを強くお勧めします。
...
Adobe Experience Platformに取り込むロイヤルティデータをモデル化する最小限のスキーマを作成しました。 このスキーマを使用すると、電子メールアドレスを使用してプロファイルを識別できます。 プロファイルのスキーマを有効にすると、ストリーミングソースのデータをリアルタイム顧客プロファイルに確実に追加できます。
...
ソースアプリケーションは、作成したスキーマに適合する、以下のようなデータをストリーミングします。
...
このデータを作成したデータセットにストリーミングするには、送信先のデータのストリーミングエンドポイントを定義する必要があります。 これをおこなうには、HTTP API ソースコネクタを定義します。

HTTP API ソースコネクタを作成するには：

1. Experience PlatformUI で、 **[!UICONTROL ソース]** under [!UICONTROL 接続] をクリックします。

2. 選択 **[!UICONTROL ストリーミング]** リストから [!UICONTROL カテゴリ].

3. 選択 **設定** 内 [!UICONTROL HTTP API] タイル。

   ![HTTP API タイルを設定](./assets/httpapi-tile.png)

4. 内 [!UICONTROL 認証] 手順 [!UICONTROL データを追加] 画面：

   HTTP API 接続の名前と説明を入力します。

   選択 **[!UICONTROL XDM 互換]** ストリーミングするデータに、既存の XDM スキーマとの互換性があることを示します。

   選択 **[!UICONTROL ソースに接続]**. 接続が成功すると、次のように表示されます。 [!UICONTROL 接続済み].

   ![HTTP API 認証](./assets/httpapi-authentication.png)

   「**[!UICONTROL 次へ]**」をクリックして続行します。

5. 内 [!UICONTROL データフローの詳細] 手順 [!UICONTROL データを追加] 画面：

   選択 **[!UICONTROL 既存のデータセット]**」で、データセットリストからデータセットを選択し、 [!UICONTROL データフロー名].

   ![データフローの詳細](./assets/httpapi-dataflowdetail.png)

   「**[!UICONTROL 次へ]**」を選択します。

6. この [!UICONTROL レビュー] 手順 [!UICONTROL データを追加] 画面に HTTP API 接続の概要が表示されます。
...
ストリーミングエンドポイント URL をコピーして使用し、ロイヤルティアプリケーションを設定し、Adobe Experience Platformロイヤルティデータセットにデータをストリーミングできます。

詳しくは、 [UI を使用した HTTP API ストリーミング接続の作成](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/streaming/http.html?lang=en) 認証の使用方法、受信データと XDM スキーマとの互換性がない場合のデータのマッピング方法、ストリーミングコネクタの設定の一環として新しいデータセットを作成する方法を説明する、より包括的なチュートリアルです。
...
>[!SUCCESS]
>
>すべての手順が完了しました。 収集するロイヤルティデータ（スキーマ）と、Adobe Experience Platform内のどこにそのデータセットを保存するか（データセット）を定義することで、まず、ロイヤルティデータを直接データセットにストリーミングする HTTP API ソースコネクタを設定しました。 データビュー定義では、使用するディメンションと指標を指定でき、最後に、最初のプロジェクトを作成し、データを視覚化および分析します。
