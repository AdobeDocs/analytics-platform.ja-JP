...
>[!NOTE]
>
>このクイックスタートガイドでは、ソースコネクタを使用してデータをAdobe Experience Platformに取り込み、Customer Journey Analyticsで使用する方法について簡単に説明します。 参照する際には、追加情報を調べることを強くお勧めします。
...
1. スキーマを作成ウィザードの「クラスを選択」ステップで、次の操作を行います。
   ...
   1. **[!UICONTROL 個人プロファイル]**&#x200B;を選択します。
      ...
      ![個人プロファイルを選択したスキーマウィンドウを作成](./assets/create-pr-schema-wizard-step-1.png)
...
1. 「スキーマの例」の「構造」タブで、次の操作を行います。
   ...
   1. 「[!UICONTROL フィールドグループ]」で「**[!UICONTROL + 追加]**」を選択します。
      ...
      ![追加フィールドグループを表示するスキーマウィンドウを作成](./assets/add-field-group-button.png)
...
1. 「[!UICONTROL フィールドプロパティ]」パネルで、名前に `Identification`、[!UICONTROL 表示名]に&#x200B;**[!UICONTROL ID]** と入力し、[!UICONTROL タイプ]で&#x200B;**[!UICONTROL オブジェクト]**、[!UICONTROL フィールドグループ]で **[!UICONTROL Profile Core v2]** を選択します。
   ...
   この識別オブジェクトは、スキーマに識別機能を追加します。 その場合は、バッチデータのメールアドレスを使用してロイヤルティ情報を識別します。
...
1. Adobe Experience Platformで、左側のパネルの[!UICONTROL CONNECTIONS]から&#x200B;**[!UICONTROL Sources]**&#x200B;を選択します。

1. 利用可能なソースコネクタのリストから、ソースコネクタを選択します。
   ...
   1. **[!UICONTROL データフローの詳細]**：名前や使用するデータセットなど、データフローに関する追加の詳細を指定します。
...
1. 各コネクタには詳細なドキュメントが用意されています。 このドキュメントにアクセスするには：
