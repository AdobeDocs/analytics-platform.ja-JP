...
* [パネルのドロップゾーン](/help/analysis-workspace/c-panels/panels.md)で作成されたアドホックフィルターは、公開されない限り、Workspace の左パネルまたはフィルターコンポーネントマネージャーに表示されません。これは、フィルターを編集し、「**[!UICONTROL このセグメントを公開する]**」を選択することで実行できます。
