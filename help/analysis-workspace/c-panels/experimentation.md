...
実験パネルは、すべてのCustomer Journey Analyticsユーザーが使用できます。 管理者権限やその他の権限は必要ありません。 ただし、[ 前提条件 ](#prerequisites) では、管理者のみが実行できるアクションが必要です。
...
![ コントロール対コントロールバリアントのパフォーマンスを示す折れ線グラフ出力。](assets/exp-output2.png)
...
1. **実験は決定的です**：実験レポートを表示するたびに、この時点まで実験に累積したデータが分析されます。 分析では、常に有効な信頼性が、バリアントの *少なくとも 1 つ* のしきい値である 95% を超えると、実験の最終的な結果が得られることを宣言します。 アームが 2 つ以上ある場合は、Benjamini-Hochberg 補正を適用して、複数の仮説テストを補正する。
