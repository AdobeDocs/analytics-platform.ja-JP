...
信頼性シーケンスは、信頼区間の「連続的な」アナログです。信頼性シーケンスとは何かを理解するには、実験を 100 回繰り返し、実験に加わる&#x200B;*新規ユーザー*&#x200B;ごとに、平均ビジネス指標（メール開封率など）とそれに関連する 95％信頼性シーケンスの推定値を計算すると想定します。
...
次の 2 つの高度な関数が追加されました。 [!UICONTROL 上昇率] および [!UICONTROL 信頼性]. 詳しくは、 [リファレンス — 高度な関数](/help/components/calc-metrics/cm-adv-functions.md).