...
>[!IMPORTANT]
>
>**[!UICONTROL クイックインサイト]** パネルは現在、制限付きテスト中です。 [詳細情報](https://experienceleague.adobe.com/docs/analytics/landing/an-releases.html?lang=ja)
...
これを初めて使用するとき [!UICONTROL Analysis Workspace]を使用すると、最も役立つビジュアライゼーションや、どのディメンションや指標がインサイトを促進し、項目をドラッグ&amp;ドロップする場所、フィルターを作成する場所などを考えることができます。

これに役立つため、および [!UICONTROL Analysis Workspace], [!UICONTROL クイックインサイト] は、会社が使用している最も人気の高いディメンション、指標、フィルターおよび日付範囲を提供するアルゴリズムを活用します。 実際、次のようにタグ付けされたディメンション、指標、フィルターが表示されます。 [!UICONTROL 人気] 次に示すように、ドロップダウンリストで使用できます。
...
* ディメンションの単純な分類、複数の指標の追加、フィルターの比較を [!UICONTROL フリーフォームテーブル].
...
| [!UICONTROL filter] | フィルターを使用すると、特性や web サイトでのインタラクションに基づいて訪問者のサブセットを識別できます。 例えば、 [!UICONTROL 訪問者] 属性に基づいてフィルタリングします。ブラウザーのタイプ、デバイス、訪問回数、国、性別またはインタラクションに基づく操作：キャンペーン、キーワード検索、検索エンジン、または出口と入口に基づくもの。facebookからの訪問者、定義済みのランディングページ、参照ドメイン、またはカスタム変数に基づく訪問者：フォームフィールド、定義済みのカテゴリ、顧客 ID。 |
...
1. 構築ブロックを選択します（コンポーネントとも呼ばれます）。ディメンション（オレンジ）、指標（緑）、フィルター（青）、日付範囲（紫）。自動的に作成されるテーブルに対して、少なくとも 1 つのディメンションと 1 つの指標を選択する必要があります。
...
* **フィルターを追加**:AND または OR 演算子を使用してテーブルに追加することで、さらに 2 つまでのフィルターを追加できます。 モバイルユーザー OR 常連訪問者を追加すると、表がどうなるかを見てみましょう。指標の上で、お互いに隣り合っています。モバイルユーザー AND 常連訪問者を追加すると、両方のフィルターの結果が一緒に表示され、表内で互いに重ね合わされます。
