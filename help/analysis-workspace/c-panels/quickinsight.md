...
この[!UICONTROL Analysis Workspace]を最初に開始したとき、最も役立つビジュアライゼーションは何か、どのディメンションと指標がインサイトを促進するか、項目をドラッグ&amp;ドロップする場所、フィルターを作成する場所などに疑問を持つかもしれません。

これを支援するため、および[!UICONTROL Analysis Workspace]での会社のデータコンポーネントの使用状況に基づいて、[!UICONTROL クイックインサイト]は、会社が使用する最も一般的なディメンション、指標、フィルターおよび日付範囲を示すアルゴリズムを活用します。 実際、次に示すように、ドロップダウンリストに、[!UICONTROL 人気のある]とタグ付けされたディメンション、指標、フィルターが表示されます。
...
* ディメンションの単純な分類、複数の指標の追加、[!UICONTROL フリーフォームテーブル]内でのフィルターの比較を簡単に行えます。
...
| [!UICONTROL filter] | フィルターを使用すると、特性やWebサイトでのインタラクションに基づいて訪問者のサブセットを識別できます。 例えば、次の属性に基づいて[!UICONTROL 訪問者]フィルターを作成できます。ブラウザーのタイプ、デバイス、訪問回数、国、性別、またはインタラクションに基づく：キャンペーン、キーワード検索、検索エンジン、または出口と入口に基づく検索：Facebookの訪問者、定義済みのランディングページ、参照ドメインまたはカスタム変数に基づくドメイン。フォームフィールド、定義されたカテゴリ、顧客ID。 |
...
1. 構築ブロック（コンポーネントとも呼ばれる）を選択します。ディメンション（オレンジ色）、指標（緑色）、フィルター（青色）または日付範囲（紫色）は、自動的に作成されるテーブルに対して、少なくとも1つのディメンションと1つの指標を選択する必要があります。
...
* **その他の追加フィルター**:ANDまたはOR演算子を使用してテーブルに追加すると、フィルターをさらに2つまで追加できます。モバイルユーザー OR 常連訪問者を追加すると、表がどうなるかを見てみましょう。指標の上で、お互いに隣り合っています。「モバイル訪問者」と「常連ユーザー」を追加すると、両方のフィルターの結果が一緒に表示され、表内で互いに重ねて表示されます。
