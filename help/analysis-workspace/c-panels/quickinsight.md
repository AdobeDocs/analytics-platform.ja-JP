...
この[!UICONTROL Analysis Workspace]の使用を初めて開始したとき、最も役立つビジュアライゼーションや、どのディメンションや指標がインサイトを容易にするか、項目をドラッグ&amp;ドロップする場所、フィルターを作成する場所などを疑問に思うかもしれません。

これに役立ち、また[!UICONTROL Analysis Workspace]での自社のデータコンポーネントの使用状況に基づいて、[!UICONTROL クイックインサイト]では、会社が使用する最も人気のあるディメンション、指標、フィルター、日付範囲を提示するアルゴリズムを活用します。 実際、次に示すように、ドロップダウンリストには、[!UICONTROL 人気]とタグ付けされたディメンション、指標、フィルターが表示されます。
...
* ディメンションの単純な分類、複数の指標の追加、[!UICONTROL フリーフォームテーブル]内でのフィルターの比較を簡単におこないます。
...
| [!UICONTROL フィルター] | フィルターを使用すると、特性やwebサイトでのインタラクションに基づいて訪問者のサブセットを識別できます。 例えば、属性に基づいて[!UICONTROL 訪問者]フィルターを作成できます。ブラウザーのタイプ、デバイス、訪問数、国、性別、またはインタラクションに基づくものキャンペーン、キーワード検索、検索エンジン、または出口と入口に基づく検索が可能です。facebookからの訪問者、定義済みのランディングページ、参照ドメイン、またはカスタム変数に基づく訪問者：フォームフィールド、定義済みのカテゴリ、顧客ID。 |
...
1. 構築ブロック（コンポーネントとも呼ばれます）を選択します。ディメンション（オレンジ）、指標（緑）、フィルター（青）、日付範囲（紫）。自動的に作成されるテーブルに対して、少なくとも1つのディメンションと1つの指標を選択する必要があります。
...
* **フィルターを追加します**。ANDまたはOR演算子を使用してテーブルに追加することで、さらに2つまでのフィルターを追加できます。モバイルユーザー OR 常連訪問者を追加すると、表がどうなるかを見てみましょう。指標の上で、お互いに隣り合っています。モバイルユーザーAND常連訪問者を追加すると、両方のフィルターの結果が一緒に表示され、表内で互いに重ね合わされます。
