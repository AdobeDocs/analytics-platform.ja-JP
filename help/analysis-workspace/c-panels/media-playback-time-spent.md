...
| 時刻の形式 | 再生に費やした時間は、次のいずれかで表示できます。 `Hours:Minutes:Seconds` （デフォルト）または `Minutes` （整数で表示され、0.5 で四捨五入されます）。 |
...
| 指標 | 説明 |
|---|---|
| 再生滞在時間 | 合計 `hours:minutes:seconds` ( または `minutes`) ：選択した精度（一時停止、バッファーおよび開始時間を含む）で表示されたコンテンツ。 |
