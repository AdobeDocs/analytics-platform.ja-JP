...
一般環境設定は、ブラウザーでのCustomer Journey Analyticsエクスペリエンスに適用されます。 これらの環境設定へのアクセス方法について詳しくは、[環境設定の更新](#update-preferences)を参照してください。
...
| | 予測を表示 | 作成する時系列のフリーフォームテーブルの最初の指標列に対して、予測値を自動的に表示するかどうかを指定します。 |
...
| | 異常値で Y 軸のスケールの設定を許可 | 予測値が履歴値の上限と下限の範囲外にある場合、y 軸はこれらの予測値に対して自動的にスケールを調整しません。 このオプションをオンにすると、予測値の Y 軸が適切にスケールされます。 |
