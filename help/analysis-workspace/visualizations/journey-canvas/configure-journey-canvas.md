...
1. 左パネルから指標、ディメンション、ディメンション項目、フィルターまたは日付範囲をキャンバスにドラッグします。 [ 派生フィールド ](/help/data-views/derived-fields/derived-fields.md) に基づく指標がサポートされます。 ただし、計算指標や、[ 概要データセット ](/help/data-views/summary-data.md) に基づく指標またはディメンションはサポートされていません。
   ...
   | 指標 | 2 つの既存のノード間の矢印 | ノードは、コンポーネントがドロップされ、両方の既存のノードに接続されている 2 つの既存のノード間に表示されます。 ( [ノードの接続](#connect-nodes) を参照してください)。</p> |
...
1. 上位ディメンション項目を表示する 2 つのノード間の矢印を右クリックします。

1. これらのノード間の最上位ノードを [!UICONTROL **表示選択します**]。

1. ディメンション項目を表示する場所を選択します。
...
ノードは「最終パス」として接続され、訪問者は、2つのノード間で発生するイベントに関係なく、最終的に一方のノードからもう一方のに移動する限りカウントされます。 ユーザーがパスに沿って移動するために割り当てられた時間は、コンテナ設定によって決定されます。<!-- It can also be controlled by [adding a time constraint](#add-a-time-constraint-between-nodes). -->

接続されたノードのロジック表示には、ノードを右クリックし、[ノード&#x200B;**]] から [[!UICONTROL **&#x200B;作成 フィルター] を選択します。ロジックについては [!UICONTROL **定義**] の節を参照してください。
...
1. 分類に使用するディメンションを選択します。
   ...
   ジャーニーキャンバスで分類表示を選択した場合は、上位 5 ディメンション項目がノードに表示されます。 ノードには、フリーフォームテーブルで分類を開くためのオプションが用意されています。
...
1. ジャーニーキャンバスビジュアライゼーションで、オーディエンスを作成する1つ以上のノードを選択し、選択したノードの1つを右クリックします。

   Or

   ジャーニーキャンバスビジュアライゼーションで、オーディエンスを作成する2つのノード間で1つ以上の矢印を選択し、選択した矢印の1つを右クリックします。
