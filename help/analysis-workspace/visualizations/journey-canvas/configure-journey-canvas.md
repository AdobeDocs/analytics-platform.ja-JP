...
ノードを選択すると、そのの直前にある上位ディメンション項目をジャーニーに表示できます。 上位 3 つのディメンション項目を個別のノードとしてジャーニーキャンバスに追加することも、フリーフォームテーブルの上位 ディメンション 項目をすべて表示することもできます。

1. ジャーニー内でそのノードより前の上位ディメンション項目を表示する場所を右クリックします。
...
1. ディメンションのリストから目的のディメンションを選択します。
   ...
   前の手順で選択した内容に応じて、上位 3 つのディメンション項目が 3 つの個別のノードとしてキャンバスに追加されるか、上位 ディメンション 項目がすべてフリーフォーム テーブルに表示されます。
...
* 左側のパネルから、複数のコンポーネントをキャンバス上のノードに同時にドラッグします。

* 左パネルから、Shiftキーを押しながら複数のコンポーネントを同時にキャンバスの空白領域にドラッグします。
...
>[!TIP]
>
>結合ノードのロジックを表示するには、ノードを右クリックして [!UICONTROL **ノードからフィルターを作成**] を選択します。 ロジックについては [!UICONTROL **定義**] の節を参照してください。
...
| 日付範囲+指標、フィルターまたはDimension | AND で結合 |
| フィルター + 指標、日付範囲、またはDimension | AND で結合 |
...
矢印に表示されるラベルと値の詳細については、 [ビジュアライゼーション設定の構成](#configure-visualization-settings)の「矢印の設定」を参照してください。
...
1. ラベルの名前を指定し、Enter キーを押します。
   ...
   ラベル非表示にするように矢印設定が現在構成されている場合は、ラベルを表示するように求めるメッセージが表示されます。

矢印の既存のラベルを更新するには:

1. ジャーニーキャンバスビジュアライゼーションで、ラベルを追加する場所の矢印を右クリックします。
...
* 分類を適用してもジャーニーは変わりません。 適用されているノードのデータの分類を表示するだけです。

* ノードに既に分類がある場合は、新しい分類を適用すると既存の置き換えられます。
...
1. ジャーニーキャンバスビジュアライゼーションで、分類を適用する 1 つ以上のノードを選択し、選択したノードのいずれかを右クリックします。
   ...
   複数のノードまたは矢印を選択するには、コマンド(Mac の場合)または Ctrl(Windows の場合)を押し続けます。
...
ジャーニーキャンバスのオブジェクトの折れ線グラフでトレンドデータを表示できます。 <!--, with some prebuilt anomaly detection data (this is the definition in Fallout) -->

トレンド表示のオプションは、キャンバス上の次のオブジェクトで使用できます。
...
1. ジャーニーキャンバスビジュアライゼーションで、トレンドデータ表示 1 つ以上のノードを選択し、選択したノードの 1 つを右クリックします。
...
1. 「 [!UICONTROL **作成ノードからフィルター**] または [!UICONTROL **作成矢印からフィルター**]&#x200B;を選択します。

   フィルタービルダーが表示されます。 [ [!UICONTROL **定義**] セクションでは、選択したノードまたは矢印と、ジャーニー内のコンテキストに基づいてフィルター定義が作成されます。

1. フィルターのタイトルを指定して、その他の変更を行います。 フィルターの作成について詳しくは、「[ フィルタービルダー ](/help/components/filters/filter-builder.md)」を参照してください。
