...
1. ビジュアライゼーションの上部に表示される次の設定を行います。
   ...
   | 設定 | 関数 |
   |---------|----------|
   | [!UICONTROL **ノードタイプ**] | ビジュアライゼーションに表示するノードタイプを設定できます。<p>ビジュアライゼーションでノードタイプを非表示にするには、ノードタイプの横にある「x」を選択するか、ドロップダウンメニューから選択を解除します。 非表示のノードタイプを表示するには、ドロップダウンメニューから選択します。 （ノードを非表示にしても、ノードはジャーニーから削除されません。 ノードの削除方法について詳しくは、[ ノードの削除 ](#delete-nodes)）を参照してください。</p><p>このフィールドで使用できるノードタイプは、次の要因によって異なります。</p><ul><li>**ジャーニーキャンバスを起源とするジャーニーの場合**、次のノードタイプを使用できます。<ul><li>ディメンション</li><li>指標</li><li>フィルター</li><li>日付範囲</li></ul></li><li>**Journey Optimizerで作成され、ジャーニーキャンバスで大幅に変更されたジャーニーの場合**、次のノードタイプを使用できます。（大幅な変更には、ノードの追加や削除、矢印の追加や削除、ノードのコンポーネントの変更などが含まれます。）<ul><li>ディメンション</li><li>指標</li><li>フィルター</li><li>日付範囲</li><li>Journey Optimizer ノードタイプ（セグメントを読み取り、終了など）</li></ul></li><li>**Journey Optimizerから開始され、ジャーニーキャンバスで大幅に変更されていないジャーニーの場合**、次のノードタイプを使用できます。（大幅な変更には、ノードの追加や削除、矢印の追加や削除、ノードのコンポーネントの変更などが含まれます。）<ul><li>Journey Optimizer ノードタイプ（セグメントを読み取り、終了など）</li></ul></li></ul> |
