...
#### シナリオ 1 - ユーザー A が最初のセッションでジャーニーパスに従うと、後続のセッションでは後続のノードにのみ従う

ユーザー A がサイトを訪問し、ジャーニーのパスを追跡したとします（ノード 1：サイトにアクセス/ノード 2：製品 A を表示/ノード 3：チェックアウト）。 このシナリオでは、ジャーニーの各ノードでイベントがカウントされます。

ここで、ユーザー A が後のセッションで再度サイトを訪問するとします。 ユーザー A は前のセッションでジャーニーパスを追跡することで、既にジャーニーの要件を満たしているので、ユーザー A がチェックアウトするたびに（ユーザー A が現在のセッションでジャーニーのパスを追跡していない場合でも）、イベントがジャーニーの 3 番目のノードである「チェックアウト」にカウントされます。 その結果、「チェックアウト」ノードのパーセンテージと数値は、前の「製品 A を表示」ノードよりも多くなります。

この例では、ジャーニーのコンテナ設定は、3 番目のノードのイベント（「チェックアウト」）が後続のセッションでカウントされるかどうかを決定する上で重要な役割を果たします。

また、セッションが（ユーザーではなく）コンテナとして設定されていた場合、後続の訪問の 3 番目のノードでのみ発生したイベントは、ジャーニーでカウントされません。ジャーニーに表示される統計は、特定のユーザーの単一の定義済みセッションに制限されるからです。 コンテナ設定について詳しくは、「ジャーニーキャンバスビジュアライゼーションの設定 [ の [ジャーニーキャンバスビジュアライゼーションの作成を開始 ](/help/analysis-workspace/visualizations/journey-canvas/configure-journey-canvas.md#begin-building-a-journey-canvas-visualization) を参照してください ](/help/analysis-workspace/visualizations/journey-canvas/configure-journey-canvas.md)

<!-- The time allotted for users to move along the path is determined by the container setting. Because "Person" is selected as the container setting in this example, people who followed the journey's path in one session (moving from Node 1 to Node 2 and to Node 3) met the criteria of the journey. On any subsequent visits to the site, any event they have that matches any node on the journey is counted on that node. -->

#### シナリオ 2 - ユーザー B がジャーニーから外れる

ユーザー B がサイトを訪問し、ジャーニーのパスに従っていない（サイトを訪問し、製品 B を表示して、チェックアウトする）場合、ジャーニーの開始ノード「サイトを訪問」でイベントがカウントされるが、残りのノードではイベントがカウントされず、ユーザー B がジャーニーから外れます。 ユーザー B は製品 A を表示してもジャーニーのパスを追跡しなかったので、ユーザー B はチェックアウトしても、イベントは 3 番目のノード「チェックアウト」にカウントされません。

これは、ユーザーがジャーニーの「最終的なパス」を進んだ場合にのみ、ノードごとにイベントがカウントされるからです。つまり、2 つのノード間でイベントが発生した場合でも、最終的にノード間で移動した期間に限ってイベントがカウントされます。
