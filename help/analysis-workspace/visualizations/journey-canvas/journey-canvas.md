...
| 機能 | 利点 |
|---------|----------|
| **イベントの作成** | 任意のイベントディメンション、指標またはフィルターを使用して、Customer Journey Analyticsを簡単に作成できます。 <p>Journey Optimizerでは、テクニカルユーザーは、ジャーニーに追加する前にイベントを作成する必要があります。</p> |
| **カスタムノードからのオーディエンスの作成** | ジャーニーキャンバスビジュアライゼーション内のジャーニーで作成したカスタムノードに基づいて、オーディエンスを作成します。 （Customer Journey Analyticsオーディエンスビルダーを起動します）。 <p>Journey Optimizerでは、事前に定義されたアクティビティに対してのみオーディエンスを作成できます。</p> |
| **フォールスルーとフォールアウト** | B3 |
| **イベントの分類** | B3 |
| **イベント名を変更** | B3 |
| **イベントの削除** | B3 |
