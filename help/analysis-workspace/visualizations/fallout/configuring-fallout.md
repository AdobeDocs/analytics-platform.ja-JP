...
タッチポイント内にフィルターを追加することもできます。 例えば、iOSユーザーや Android ユーザーなどのフィルターを比較する場合があります。 目的のフィルターをフォールアウトの上部にドラッグすると、それらのフィルターに関する情報がフォールアウトレポートに追加されます。 これらのフィルターのみを表示する場合、「すべての訪問」ベースラインを削除できます。
...
1. シーケンスが完成するまでタッチポイントの追加を続けます。
   ...
   >[!NOTE]
   >
   >複数のフィルターは AND で結合されますが、ディメンション項目や指標などの複数の項目は OR で結合されます。
