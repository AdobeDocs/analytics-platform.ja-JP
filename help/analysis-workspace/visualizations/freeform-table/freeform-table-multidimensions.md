...
複数のディメンション列を持つフリーフォームテーブルにフィルター、並べ替え、分類などを適用して、より深くカスタムの分析を作成できます。

## 連結されたディメンション項目

複数のディメンション列をフリーフォームテーブルに追加する場合、ディメンション項目の各行は、単一の連結ディメンション項目のように動作します。 この機能を使用すると、特定のディメンションの組み合わせに対する指標データを表示できます。

例えば、ディメンションが _市区町村_、_デバイスタイプ_、および _日_ で、指標が _イベント_ であるフリーフォームテーブルについて考えてみます。 このテーブルの最初の行にある 3 つのディメンション項目は、月の 30 日に携帯電話からムンバイで発生したイベントが 2,056 件あったことを示す単一の連結ディメンション項目になります。
...
1. テーブルの各行を単一のディメンション項目として表示します。 詳しくは、「[ 連結されたディメンション項目の表示 ](#view-concatenated-dimension-items)」を参照してください。
