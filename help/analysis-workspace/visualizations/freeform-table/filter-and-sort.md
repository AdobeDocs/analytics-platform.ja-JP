...
Analysis Workspaceでは、ディメンションか指標かに関わらず、任意の列でフリーフォームテーブルのデータを並べ替えることができます。 複数の列で同時に並べ替えることもできます。
...
複数の列のデータを並べ替える場合、各列に割り当てた優先度に従ってデータが並べ替えられます。 優先度番号は、並べ替えアイコン ![SortOrderDown](/help/assets/icons/SortOrderDown.svg)<sup>➊</sup> の横に表示されます。
