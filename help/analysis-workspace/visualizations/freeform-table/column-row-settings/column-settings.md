---
description: 列の設定を編集して列の書式を設定する方法について説明します。一部は条件付き書式にすることができます。
title: 列設定
...
| 条件付き書式オプション | 説明 |
| --- | --- |
| **[!UICONTROL パーセント制限を使用]** | 絶対値ではなくパーセンテージに基づいて制限範囲を変更します。割合制限範囲は、割合にのみ基づく指標（バウンス率など）と、カウントと割合を持つ指標（ページビュー数など）に対して機能します。 |
