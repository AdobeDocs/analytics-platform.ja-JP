...
* **[!UICONTROL テーブルの合計]** ➊ - この合計は通常、[!UICONTROL 総計]と等しいか、そのサブセットです。合計には、「なし [!UICONTROL &#x200B; オプションを含む &#x200B;] など、フリーフォームテーブル内で適用されたすべてのテーブルセグメントが反映されます。
* **[!UICONTROL 総計]**（**[!UICONTROL 範囲外の]***数*）➋ - この合計は、収集されたすべてのイベントを表します。 セグメントがパネルレベルまたはフリーフォームテーブル内で適用されると、この合計は、セグメントの条件に一致するすべてのイベントを反映するように調整されます。
