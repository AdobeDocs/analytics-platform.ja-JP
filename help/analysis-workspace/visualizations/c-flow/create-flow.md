...
新しい[!UICONTROL フロー]エクスペリエンスは、いくつかの方法でワークフローを強化します。

* 指標とパスディメンションを組み合わせて、パスの開始または終了を選択できるようになりました。
* [!UICONTROL フロー]をさらに細かくカスタマイズできる[!UICONTROL 詳細設定]が含まれています。
* 新しい「ビルド」ボタンを使用すると、ジャーニーを一度に設定し、クエリを実行して、複数の列とノードを一度で自動的に構築できるので、分析にかかる時間を節約できます。
...
1. （オプション）「**[!UICONTROL 詳細設定を表示]**」をクリックして詳細設定を構成します。
   ...
   | **[!UICONTROL 最初／最後の発生件数に制限]** | パスを、ディメンション／項目／指標の、最初／最後の出現で始まる／終わるパスに制限します。詳しくは、以下の「最初/最後のオカレンスに制限するシナリオの例」の節を参照してください。 |
