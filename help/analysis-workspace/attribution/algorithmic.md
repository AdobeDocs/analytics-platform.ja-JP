...
See ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [Algorithmic attribution](https://experienceleague.adobe.com/ja/docs/analytics-learn/tutorials/analysis-workspace/attribution-iq/algorithmic-model-in-attribution-iq){target="_blank"} for a demo video.

>[!ENDSHADEBOX]

-->

アトリビューションに用いられるアルゴリズムは、協同ゲーム理論のハルサニ配当に基づきます。 Harsanyiの配当金は、結果に不平等な貢献を持つゲームのプレイヤー間で信用を分配するためのShapleyの価値ソリューションの一般化（Lloyd Shapley、ノーベル賞経済学者にちなんで名付けられた）です。
...
>[!NOTE]
>
>アルゴリズムアトリビューションの結果が他のモデルと異なるのは、指定されたルックバックウィンドウ内に複数のタッチポイントが存在する場合のみです。 単一の顧客接点を持つコンバージョンは、アトリビューションモデルに関係なく、100% のクレジットを受け取ります。
