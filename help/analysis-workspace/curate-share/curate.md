...
1. （任意）他のコンポーネントを追加するには、共有するコンポーネントを左のパネルから「[!UICONTROL  コンポーネントをキュレーション ]」フィールドにドラッグします。
...
左側のパネルでプロジェクトのキュレーションを削除し、コンポーネントの完全なセットを復元するには：
...
キュレートされたプロジェクトでは、受信者に、左側のパネルに **[!UICONTROL すべて表示]** コンポーネントを表示するオプションが表示されます。 「[!UICONTROL すべてを表示]」では、以下に応じて異なるコンポーネントセットを表示します。
