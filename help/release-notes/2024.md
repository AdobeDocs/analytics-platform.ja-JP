...
| **Adobe Product Analytics：ファネルのインサイトの強化** | [ ファネル ](/help/guided-analysis/types/funnel.md) ビューでインサイトの記述が強化され、カテゴリ、差分および説明が追加されて、グラフとテーブルがよりわかりやすくなりました。 | 2024年4月17日（PT） | 2024年4月26日（PT） |
| **Adobe Product Analytics：リテンションビューの強化** | [ 保持 ](/help/guided-analysis/types/retention.md) ビューに複数の機能が追加され、より深くカスタマイズ可能な保持インサイトが促進されました。<ul><li>開始および再来訪イベントをリンク解除</li><li>1 つのビューで複数のリターンイベントを比較</li><li>設定時または設定後（無制限）、各設定（制限付き）およびブラケット設定で適用されるリテンションモデルをカスタマイズ</li><li>グラフ内の個々のコホート行を表示／非表示</li></ul> | 2024年4月24日（PT） | 2024年5月8日（PT） |
