...
| **Customer Journey Analytics 用 MCP サーバー** <br/>MCP（モデルコンテキストプロトコル）を使用することで、Customer Journey Analytics を既存のエージェント型ワークフローに関連付けることができるようになりました。自然言語を使用して、レポートやインサイトをリクエストできます。<p>（ドキュメントのリンクは以下を参照。）</p> | | 2026年4月末 |
