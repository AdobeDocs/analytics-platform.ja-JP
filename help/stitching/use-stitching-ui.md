---
title: ステッチを有効にする
description: Customer Journey AnalyticsでイベントデータセットのID合成を有効にします。 接続UIで永続ID、人物ID、再生ウィンドウを設定してデータをつなぎ合わせる方法について説明します。
...
接続の一部として設定した1つ以上のイベントデータセットでステッチを有効にできます。 ライセンスを取得したCustomer Journey Analytics パッケージによって、ステッチで有効にできるイベントデータセットの数が決まります。
...
* 永続的なIDまたは人物IDにXDM スキーマフィールドを使用する場合は、イベントデータセットのスキーマでIDが適切にマークされていることを確認します。 [ID名前空間の概要](https://experienceleague.adobe.com/ja/docs/experience-platform/identity/features/namespaces)を参照してください。
...
>additional-url="https://experienceleague.adobe.com/en/docs/analytics-platform/using/technotes/badids" text="不正 ID"


### データセット設定

結合を有効にするには、**[!UICONTROL データセットを追加]**&#x200B;または&#x200B;**[!UICONTROL データセットを編集]** ダイアログのイベントデータセット **[!UICONTROL データセット設定]** セクションで次の操作を行います。

![ID ステッチを有効にする際のID ステッチ オプション &#x200B;](assets/identity-stitching-ui.png)
...
1. 「**[!UICONTROL 永続ID]**」ドロップダウンメニューから永続IDを選択します。
   ...
   永続的IDに「**[!UICONTROL ID マップ]**」を選択した場合は、名前空間を選択する必要があります。 選択肢は次の2つです。
...
1. 「**[!UICONTROL 人物ID]**」ドロップダウンメニューから人物IDを選択します。
   ...
   ユーザーIDに「**[!UICONTROL ID マップ]**」を選択した場合は、名前空間を選択する必要があります。 選択肢は次の2つです。
...
>[!NOTE]
>AWSにCustomer Journey Analyticsをデプロイしたお客様の場合、この機能は保留中のリリースです。
>

![ID ステッチを有効にする際のID ステッチ オプション &#x200B;](assets/identity-stitching-ui-preview.png)
...
最終的に、Adobeは、リクエストベースのステッチされたデータセットを、接続の新しいステッチのエクスペリエンスに移行します。
