---
title: グラフベースのステッチ
description: グラフベースのステッチの概念と作業について説明します
...
グラフベースのステッチでは、イベントデータセットと、そのデータセットの人物 ID の永続 ID （cookie）および名前空間を指定します。 グラフベースのステッチでは、ステッチ ID の新しい列がイベントデータセットに追加されます。 次に、永続 ID を使用して、指定された名前空間を使用してExperience Platform ID サービスから ID グラフをクエリし、ステッチされた ID を更新します。
...
ルックバックウィンドウを超えたデータは再生されません。未認証の訪問と認証済みの訪問を一緒に識別するには、プロファイルが、所定のルックバックウィンドウ内で認証される必要があります。 デバイスが認識されると、その時点からライブステッチされます。
...
- Experience Platform ID サービスの ID グラフには、`Email` ユーザー ID`Phone` を解決するためにステッチ中に使用する名前空間（例：**または**）が必要です。 詳しくは、[Experience Platform ID サービス &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-platform/identity/home) を参照してください。
