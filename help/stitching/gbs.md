...
![ID サービス](assets/uis-gbs.png){zoomable="yes"}
...
2023-05-13 16:30にリプレイ合成が行われ、24時間のルックバックウィンドウ設定が行われると、サンプルの一部のイベントが再ステッチされます（![Replay](/help/assets/icons/Replay.svg)で示されます）。
...
2023-05-13 16:30にリプレイステッチが発生し、7日間のルックバックウィンドウ設定で、サンプルのすべてのイベントが再ステッチされます。
...
次の表は、上記と同じデータを表していますが、サンプルイベントに対するプライバシーリクエスト（例：2023-05-13 18:00）の影響を示しています。
...
- 指定された名前空間を使用してユーザー ID をクエリする場合、タイムスタンプは考慮されません。 そのため、永続 ID が、以前のタイムスタンプを持つレコードのユーザー ID とステッチされる可能性があります。
...
- ID グラフに ID をバックフィルできる期間は 3 か月までという厳密な制限があります。 Real-time Customer Data Platform などの Experience Platform アプリケーションを使用して ID グラフに ID を取り込んでいない場合は、ID のバックフィルを使用します。
- [ID サービスのガードレール](https://experienceleague.adobe.com/ja/docs/experience-platform/identity/guardrails)が適用されます。 例えば、次の[静的制限](https://experienceleague.adobe.com/ja/docs/experience-platform/identity/guardrails#static-limits)を参照してください。
  - グラフ内の ID の最大数：50。
  - 1 回のバッチ取り込みで、1 つの ID にリンクできる最大数：50。
  - グラフ取り込みでの XDM レコード内の ID の最大数：20。
  - グラフ取り込みでの XDM レコード内の ID の最小数：2。
