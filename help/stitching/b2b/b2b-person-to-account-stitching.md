...
B2Bの人物とアカウントの結合をサポートするには、[B2Bの結合設定](#configure-b2b-person-to-account-stitching-settings)を設定する際に、メインの人物ID名前空間（電子メールなど）と人物からアカウントへのマッピングデータセットを指定します。
...
グラフベースの合成は、エクスペリエンスイベントデータセットの人物IDを昇格するためにも使用されます。 例えば、[ データセット ](#enable-b2b-person-to-account-stitching-on-event-datasets)でステッチを有効にする際に、永続的なユーザーID （ECID）フィールドを永続的なユーザーIDとして使用するように設定します。 昇格された人物からアカウントへのマッピング データセット `emily@adobe.com`に基づいて、関連するイベントで昇格された人物IDとして設定されます。
...
個人からアカウントへのデータセットは、エクスペリエンスイベントデータセットのアカウント IDを昇格するために再度使用されます。 例えば、emily@sky.comの場合は&#x200B;**Sky**、carmen@adobe.comとemily@adobe.comの場合は&#x200B;**Adobe**&#x200B;の付加値を参照してください。 cory@sky.comの値&#x200B;**Sky** （Ubiquityから）が更新されました。
...
1. **[!UICONTROL B2B ステッチ設定]** ダイアログで、次の操作を行います。
   ...
   1. 「**[!UICONTROL ユーザーからアカウント]**」の下の「**[!UICONTROL アカウント]**」セクションを設定します。
      ...
      | **[!UICONTROL ユーザー ID]** | ![必須](/help/assets/icons/Required.svg) | ユーザー ID を含むデータセット内のフィールドを選択します。 このフィールドの名前空間は、選択したユーザー識別子名前空間と異なるか、同じである場合があります。 異なる場合は、2 つの名前空間を ID グラフでリンクする必要があります。  このフィールドはIDとしてマークする必要があり、**[!UICONTROL アカウント ID]** フィールドまたは&#x200B;**[!UICONTROL 開始時間]** フィールドと同じにすることはできません。 |
      | **[!UICONTROL アカウント ID]** | ![必須](/help/assets/icons/Required.svg) | 一意のアカウント識別子の値を含むデータセットのフィールドを選択します。 ユーザーとアカウントのステッチが有効になっているイベントデータセットの各行で、アカウント ID 情報が使用可能になります。 このフィールドは、**[!UICONTROL 人物ID]** フィールドまたは&#x200B;**[!UICONTROL 開始時間]** フィールドと同じにすることはできません。 |
...
>abstract="有効にすると、このデータセットは B2B ユーザーとアカウントのステッチを使用します。 **[!UICONTROL 永続的な人物ID]**&#x200B;値は、設定された&#x200B;**[!UICONTROL 人物ID名前空間]**&#x200B;から値に昇格され、その後、人物からアカウントへのデータセットに基づいてアカウント IDを検索するために使用されます。<br/>無効にした場合、このデータセットではB2B ユーザーからアカウントへのステッチは使用されないので、代わりに必須の&#x200B;**[!UICONTROL アカウント ID]**&#x200B;を選択する必要があります。"
...
* 人物IDの設定が必要です。 その人物IDは、[人物からアカウントへのデータセット ](#prerequisites)に基づいてアカウント IDを検索するために使用されます。
* アカウント IDの設定はオプションです。
