...
>[!NOTE]
>このページのスコアカードのスクリーンショットは、CJA ではなく、Adobe Analytics UI から取得されました。 UI はほぼ同じです。
...
1. 正しいタイルを選択します。
1. 右側のレールの「**[!UICONTROL ドリルイン]**」で、削除する **[!UICONTROL 未指定]** 項目を持つディメンション項目の横にある右矢印を選択します。

   ![未指定](assets/unspecified.png)

1. 「**[!UICONTROL 未指定]**」の横のアイコンをクリックして、未指定のデータをレポートから削除します。 （他のディメンション項目も削除できます）。
...
タイルをクリックすると、動的なポップアップに、アプリ内のエグゼクティブユーザーにドリルイン（分類）ビューがどのように表示されるかが表示されます。 ディメンションおよびディメンション項目を分類して、特定のニーズに合わせてデータを分類できます。 ディメンションがタイルに適用されていない場合、分類ディメンションは、デフォルトの日付範囲に応じて、**時間**&#x200B;または&#x200B;**日**&#x200B;になります。

分類を使用すると、次の小売の例のように、ディメンションを他の指標やディメンションで逐語的に分類して、分析を絞り込むことができます。
