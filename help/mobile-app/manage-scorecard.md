---
description: Analytics ダッシュボードのスコアカードの表示および管理方法
title: スコアカードの管理
...
このトピックでは、Customer Journey Analytics データのキュレーターに、スコアカード要素の表示と管理方法について説明します。
...
タイルに追加されたディメンションは、アプリの詳細ビューのドロップダウンメニューに表示されます。 エグゼクティブユーザーは、ドロップダウンメニューにリストされているオプションの中から選択できます。
...
例えば、web トラフィックに焦点を当てると、最も重要な指標は訪問数ですが、新規人物数や一意の人物数にも関心がある場合や、web ページ別またはトラフィックが発生しているデバイスタイプ別にデータを分類したい場合があります。 モバイルスコアカードプロジェクトのデータストーリーを使用すると、最も重要な指標を前面と中央に配置しながら、指標の背後にあるストーリー全体を複数の詳細スライドで伝えることができます。
...
デモ動画については、![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [&#x200B; モバイルスコアカードプロジェクトのデータストーリー](https://experienceleague.adobe.com/ja/docs/customer-journey-analytics-learn/tutorials/dashboards/create-a-mobile-scorecard){target="_blank"}を参照してください。
...
## スコアカードでのインテリジェントなキャプションの表示 {#captions}

インテリジェントなキャプションは、アナリスト以外の人でも、アナリストの助けを借りずにデータをより深く理解するのに役立ちます。 インテリジェントなキャプション高度なマシンラーニング（機械学習）と生成AIを使用して、ビジュアライゼーションに役立つ自然言語のインサイトを獲得できます。

これらのキャプションは、データ内の重要な統計的インサイトを選択し、自然言語で表現することで、アナリスト以外の人がデータリテラシーを身につけられるように支援します。

インテリジェントなキャプションは、折れ線、ドーナツ、棒グラフ、概要番号など、モバイルアプリのすべてのビジュアライゼーションで使用できます。

モバイルスコアカードでインテリジェントなキャプションを表示するには：
...
1. インテリジェントなキャプションをすべて表示するには、右にスワイプします。
   ...
   インテリジェントなキャプションは、データの急増、異常値、傾向、減少、相関関係などの現象を示すことができます。
