...
このトピックでは、Customer Journey Analyticsデータのキュレーターにスコアカード要素の表示および管理方法を示します。
...
同様に、スコアカード全体に適用されるコンポーネントを削除するには、タイルの外側のスコアカード上の任意の場所をクリックし、 **x** コンポーネントの上にマウスポインターを置くと表示されます。以下に例を示します **初回訪問**:
...
1. が含まれる **[!UICONTROL モバイルスコアカードの共有]** フォームで、次の方法でフィールドに入力します。
...
1. リンクをコピーして、メール、ドキュメント、または IM アプリに貼り付けます。
   ...
   受信者がデスクトップアプリまたはブラウザーを使用してリンクを開くと、モバイルスコアカードプロジェクトがワークスペースで開きます。
...
1. X をクリックして終了します。 **[!UICONTROL レポート結果]** ダイアログ。
