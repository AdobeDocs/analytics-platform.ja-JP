...
The **[!UICONTROL 定着率]** 「表示」は、製品の市場適合を把握するのに役立つ、製品の使用状況の繰り返し動作を時間の経過と共にユーザーに表示します。 この表示では、横軸はユーザーの最初のエンゲージメントからの日数を表し、縦軸は再びエンゲージしたユーザーの割合を表します。
...
* **製品市場の適合**：製品の通常の使用状況を測定し、リテンション曲線として視覚化します。 保持率が高いと、製品市場の適合率が高くなり、曲線が平らになる場所は、適合に到達するまでにかかる時間を示します。 この分析を全体的なレベルで表示したり、個々の製品機能別に分類したりして、より深いインサイトを得たりできます。
* **購読サービスの分析**：製品がサブスクリプションまたは他のタイプの定期的な売上高モデルを採用している場合、製品を最大限に活用しているユーザーの割合を表示できます。 ユーザーが示す特定の品質や行動を特定できます。
...
* **[!UICONTROL イベントを開始してリターン]**：分析に含める条件や定義の条件としてユーザーが関与する必要があるイベント条件。 1 つのイベントがサポートされていますが、プロパティフィルターを含めることができます。
...
* **[!UICONTROL グラフのタイプ]**：使用するビジュアライゼーションのタイプです。 次のオプションがあります [!UICONTROL 棒グラフ] および [!UICONTROL 線]. 折れ線グラフのビジュアライゼーションでは、 Day 0 がグラフに視覚的に表示されます。
...
* **[!UICONTROL 間隔]**：保持データを表示する日付精度。 有効なオプションは、日別、週別、月別です。 同じ日付範囲に異なる間隔を設定することができ、期間バケットのオプションに影響を与えます。
