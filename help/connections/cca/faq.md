...
[Cross-Channel Analytics](../overview.md) は、人がどのようにデバイスとチャネルの両方をまたいで操作するかを把握できる、CJA 専用の機能です。データセットの人物 ID を再入力することで、そのデータセットを他のデータセットとシームレスに組み合わせることができます。この機能は、CDA のフィールドベースのステッチと同様の設計で動作しますが、従来の Analytics と CJA ではデータアーキテクチャが異なるので、実装は異なります。
