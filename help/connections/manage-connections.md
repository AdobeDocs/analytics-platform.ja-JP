...
### Customer Journey AnalyticsでのJourney Optimizer接続の使用 {#use-connection-in-cja}
...
Customer Journey Analyticsにアクセスできるようになったら、Customer Journey AnalyticsでJourney Optimizer接続を使用できるようにする必要があります。

1. Customer Journey Analyticsで使用するJourney Optimizer接続を見つけます。

   1. 「**[!UICONTROL 接続]**」タブのフィルターアイコンを選択します。

   1. **[!UICONTROL CJAで使用]** セクションで、「**[!UICONTROL オフ]**」を選択します。

      Customer Journey Analyticsで使用するように現在設定されていないすべての接続が表示されます。

   1. Customer Journey Analyticsで使用するJourney Optimizer接続を選択します。

1. Journey Optimizer接続で、「**[!UICONTROL CJAで使用]**」を選択します。

   次の **[!UICONTROL この接続をCustomer Journey Analyticsで使用]** ダイアログが表示されます。

   ![ 接続での使用 ](assets/use-in-connection.png)

1. **[!UICONTROL CJAで接続を使用]** トグルを有効にします。

1. 「**[!UICONTROL 接続を使用]**」を選択します。<!-- double-check these dialog button names -->

#### Customer Journey Analyticsからの接続の削除 {#remove-connection-in-cja}
...
1. Journey Optimizer接続を編集する場合は、「**[!UICONTROL CJAから削除]**」を選択します。

   次の **[!UICONTROL Customer Journey Analyticsからこの接続を削除]** ダイアログが表示されます。

   ![ 接続での使用 ](assets/remove-connection.png)
...
>title="平均行サイズ"
>abstract="前月と比較した変化率を含む、今月のデータ行の平均サイズ。"


>[!CONTEXTUALHELP]
>id="connections_coredatavolume"
>title="コアデータボリューム"
>abstract="今月のコアデータのボリューム。*正確な定義の確認が必要です。*"
