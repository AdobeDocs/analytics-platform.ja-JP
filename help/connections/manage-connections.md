...
| [!UICONTROL レコード] 削除済み | **選択したデータセットと日付範囲**&#x200B;に対して、選択した期間に削除された行数を示します。たとえば、Experience Platform 内のデータセットが削除された可能性があります。10 分ごとに更新されます。<p>一部のシナリオでは、この値には、ステッチや一部のルックアップデータセットの更新に伴うレコードの置き換えも含まれます。 次の例を考えてみましょう。</p><ul><li>1 つのレコードを XDM 個別プロファイルデータセットにアップロードします。CJA は、プロファイル参照データとして取り込むように設定されます。 接続の詳細では、このデータセットには 1 件のレコードが追加されて表示されます。</li><li>元のレコードの重複を同じ AEP データセットにアップロードします。このデータセットには、2 つのレコードが含まれます。 CJA は、プロファイルルックアップデータセットから追加のレコードを取り込みます。 CJA は、そのユーザー ID の接続に既にプロファイルレコードを取り込んでいることを確認すると、以前のバージョンを削除して、新しいプロファイルデータを追加します。 CJA は取り込まれたユーザー ID の最新のプロファイル参照データのみを保持するので、接続の詳細では、追加された 1 件のレコードと削除された 1 件のレコードが含まれます。</li><li>合計で、AEP データセットには、同じである 2 つのレコードが含まれます。 CJA 接続の詳細には、取り込んだデータのステータスが別々に表示されます。取り込んだデータのステータスは、このプロファイルデータセットに対して追加された 2 件のレコードと削除された 1 件です。 </li></ul> |
