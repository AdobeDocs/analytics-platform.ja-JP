...
>title="平均行サイズ"
>abstract="取り込まれてCustomer Journey Analyticsに保存されたデータの各行によって消費されたストレージの平均量（KB 単位）。当月は、前月と比較して変化した割合を使用します。"


>[!CONTEXTUALHELP]
>id="connections_coredatavolume"
>title="コアデータボリューム"
>abstract="今月のタイムスタンプが付いた、ディスクに保存されているデータの合計量（GB または TB 単位）。前月と比較した変化の割合です。"
