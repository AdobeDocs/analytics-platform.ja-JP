...
1. 「**[!UICONTROL プロファイルデータセット]**」セクションで、報告する同意ポリシーメンバーシップデータ（`consentPoliciesIDMap` フィールド）を含むプロファイルデータセットを選択します。 このプロファイルデータセットは、選択した接続に追加されます。
...
1. （オプション）「**[!UICONTROL フィルタリング]**」セクションでは、次のマーケティングアクションのフィルタリングを有効にできます。
   ...
   >[!NOTE]
   >
   >フィルタリングなしで同意レポートを有効にするには、両方の切り替えスイッチをオフのままにします。
   >
...
1. **[!UICONTROL 作成]**&#x200B;を選択して、設定を作成します。
   ...
   Customer Journey Analyticsの自動処理：
   ...
   * 新しいコンポーネントに&#x200B;**consent**&#x200B;内部ラベルを適用して、データビューでフィルター処理できるようにします。 内部ラベルについて詳しくは、[&#x200B; ラベルとポリシー](/help/data-views/data-governance.md)を参照してください。
