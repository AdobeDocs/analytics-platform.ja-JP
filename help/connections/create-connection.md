...
2022 年 5 月に、Customer Journey Analytics(CJA) で新しい接続ワークフローが開始されます。 新機能の概要を次に示します。
...
1. 接続を設定します。
   ...
   | **[!UICONTROL 周期データウィンドウを有効にする]** | この設定を使用すると、CJA データ保持を、接続レベルで、月（1 か月、3 か月、6 か月など）単位の周期的な期間として定義できます。<p>データ保持は、イベントデータセットのタイムスタンプに基づいており、イベントデータセットにのみ適用されます。該当するタイムスタンプがないので、プロファイルまたは参照データセットには、周期的なデータウィンドウ設定は存在しません。 ただし、接続に（1 つ以上のイベントデータセットに加えて）任意のプロファイルまたは参照データセットが含まれる場合、そのデータは同じ期間保持されます。<p> 主な利点は、該当する有用なデータのみを保存またはレポートして、有用でなくなった古いデータを削除できるという点です。契約上の上限を超えないようにし、超過コストのリスクを軽減します。 |
   ...
   | **[!UICONTROL 最終更新日]** | イベントデータセットの場合のみ、この設定は、イベント内のイベントベースのスキーマから、デフォルトのタイムスタンプフィールドに自動的にExperience Platformされます。 「N/A」は、このデータセットにデータが含まれていないことを意味します。 |
